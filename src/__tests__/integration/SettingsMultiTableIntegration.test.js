        ...
        this.simulateMessage({
          ...
          payload: {
            settings: {
              ...
              mlModelConfigs: message.payload.settings.mlParameters ? [message.payload.settings.mlParameters] : [],
              version: {
                id: 'version-migrated',
                versionNumber: 1,
...
describe('Settings Multi-Table Backend Integration', () => {
  ...
  describe('Complete CRUD Flow', () => {
    ...
    test('DELETE: Remove priority rule from priority_rules table', async () => {
      ...
      await act(async () => {
        await result.current.updatePriorityRules({ id: 'rule-1', deleted: true });
      });
  ...
  describe('Version Control', () => {
    test('creates new config version', async () => {
      ...
      await waitFor(() => {
        ...
        expect(result.current.version?.description).toBe('Pre-migration backup');
      });
    });
  ...
  describe('Cross-Table Data Consistency', () => {
    test('maintains referential integrity between staff_groups and backup assignments', async () => {
      ...
      await waitFor(() => {
        expect(result.current.settings?.staffGroups[0].members).toContain('staff-1');
      });
    });
  ...
  describe('Migration Workflow', () => {
    test('migrates localStorage settings to multi-table backend', async () => {
      ...
      const localStorageSettings = {
        staffGroups: [
          { id: 'group-1', name: 'Kitchen', members: ['staff-1'] }
        ],
        dailyLimits: [
          { id: 'limit-1', maxShifts: 2 }
        ],
        monthlyLimits: [],
        priorityRules: [],
    ...
    test('preserves data integrity during migration', async () => {
      ...
      const complexSettings = {
        ...
        monthlyLimits: [
          { id: 'ml1', limitConfig: { maxMonthlyShifts: 20 } }
        ],
        priorityRules: [],
