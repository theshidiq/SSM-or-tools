...
async function loadWorkerModels(config) {
  try {
    ...
    if (config.enableConstraintML) {
      const constraintModel = await createConstraintModel(config.constraintConfig);
  ...
  try {
    ...
    for (let staffIndex = 0; staffIndex < staffMembers.length; staffIndex++) {
      ...
      for (let dateIndex = 0; dateIndex < dateRange.length; dateIndex += chunkSize) {
        const dateChunk = dateRange.slice(dateIndex, Math.min(dateIndex + chunkSize, dateRange.length));
        
        // Process chunk with yielding
        await processChunkWithYielding(staff, dateChunk, scheduleData, results, options);
        
        processedCount += dateChunk.length;
        const progress = Math.min(95, Math.floor((processedCount / totalCells) * 90) + 5);
    ...
    await validateAndOptimizeResults(results, scheduleData, staffMembers, dateRange);
 ...
 */
async function processChunkWithYielding(staff, dateChunk, scheduleData, results, options) {
    ...
    try {
      ...
      if (features && mlModels.has('prediction')) {
        ...
        if (prediction) {
          ...
          results.confidence.set(`${staff.id}_${dateKey}`, prediction.confidence);
 ...
 */
async function validateAndOptimizeResults(results, scheduleData, staffMembers, dateRange) {
  ...
  for (const [key, prediction] of results.predictions) {
    ...
    if (staff) {
      ...
      if (prediction === '×' && violatesOffDayLimits(staff, dateKey, scheduleData, results.predictions)) {
...
function violatesOffDayLimits(staff, dateKey, scheduleData, predictions) {
  ...
  if (scheduleData[staff.id]) {
    for (const [key, shift] of Object.entries(scheduleData[staff.id])) {
      ...
      if (keyDate.getMonth() === month && keyDate.getFullYear() === year && shift === '×') {
        offDayCount++;
      }
    }
  }
...
function setupWorkerConfigurations(config) {
  ...
  if (config.memoryLimitMB) {
    ...
    tensorMemoryTracker.cleanupThreshold = Math.floor(tensorMemoryTracker.maxMemoryUsage * 0.8);
  }
}