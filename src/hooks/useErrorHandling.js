...
export const useErrorHandling = (options = {}) => {
  ...
  const classifyError = useCallback((error, context = {}) => {
    const errorInfo = {
    ...
    } else if (error.name === 'ValidationError' || context.type === 'validation') {
      errorInfo.category = 'validation';
      errorInfo.severity = 'low';
      errorInfo.recoverable = false;
    } else if (error.name === 'DatabaseError' || context.type === 'database') {
      errorInfo.category = 'database';
      errorInfo.severity = 'high';
      errorInfo.recoverable = true;
    } else if (error.name === 'AuthenticationError' || context.type === 'auth') {
  ...
  const handleError = useCallback(async (error, context = {}) => {
    ...
    return errorInfo;
  }, [classifyError, enableAutoRecovery, enableUserNotifications, onError, systemHealth]);

  // Recovery attempt system
  const attemptRecovery = useCallback(async (errorInfo, context = {}) => {
    const recoveryKey = `${errorInfo.category}-${errorInfo.type}`;
    const attempts = recoveryAttempts.get(recoveryKey) || 0;

    if (attempts >= maxRetryAttempts) {
      consoleLogger.warn('Max recovery attempts reached', { errorInfo, attempts });
      escalateError(errorInfo);
      return false;
    }

    // Update attempts
    setRecoveryAttempts(prev => new Map(prev.set(recoveryKey, attempts + 1)));
    ...
    try {
      ...
      if (recovered) {
        consoleLogger.info('Error recovery successful', { errorInfo, attempts: attempts + 1 });
  ...
  const applyRecoveryStrategy = useCallback(async (errorInfo, context) => {
  ...
  const recoverNetworkError = useCallback(async (errorInfo, context) => {
    try {
      ...
      consoleLogger.debug('Network still unavailable', { error: error.message });
      return false;
    }
  }, []);
  ...
  const updateSystemHealth = useCallback((errorInfo) => {
    setSystemHealth(current => {
      const criticalErrors = Array.from(errors.values()).filter(e => e.severity === 'critical').length;
      const highErrors = Array.from(errors.values()).filter(e => e.severity === 'high').length;
  ...
  const escalateError = useCallback((errorInfo) => {
    consoleLogger.error('Error escalated - manual intervention required', { errorInfo });
    ...
    if (enableErrorReporting && typeof window !== 'undefined') {
      ...
      try {
        ...
        console.warn('Error escalation would be reported to external service:', errorInfo);
      } catch (reportingError) {
        consoleLogger.debug('Failed to report escalated error', { reportingError });
      }
    }
  ...
  const showUserNotification = useCallback((errorInfo) => {
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('app-error-notification', {
  ...
  const getErrorStats = useCallback(() => {
    ...
    return {
      ...
      systemHealth,
      recoveryAttempts: Array.from(recoveryAttempts.values()).reduce((a, b) => a + b, 0),
      escalatedErrors: historyArray.filter(e => errorCounts.current.get(`${e.type}-${e.message}`) >= escalationThreshold).length
    };
  }, [errors, errorHistory, systemHealth, recoveryAttempts, escalationThreshold]);
  ...
  const withErrorHandling = useCallback((asyncOperation, context = {}) => {
    return async (...args) => {
      try {
