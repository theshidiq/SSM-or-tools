 ...
 *
 * Key Features:
 * - Real-time staff data fetching from app_staff table
...
export const useStaffManagementNormalized = (currentMonthIndex, options = {}) => {
  ...
  /**
   * Load staff data using normalized architecture with period filtering
   */
  const {
    data: rawStaffData,
    ...
    queryKey: NORMALIZED_STAFF_QUERY_KEYS.staffForPeriod(currentMonthIndex),
    queryFn: async () => {
      console.log(
        `🔍 Loading staff data using normalized architecture for period ${currentMonthIndex}...`,
      );

      try {
        // Use the database function to get staff for the current period
        ...
        console.log(`📊 Loaded staff data from normalized architecture:`, {
        ...
        // Fallback to direct app_staff query if the function fails
        console.log("🔄 Falling back to direct app_staff query...");
        ...
        console.log(`📊 Loaded fallback staff data: ${fallbackData?.length || 0} total staff`);
        return fallbackData || [];
      }
    },
    enabled: isConnected,
  ...
  const staffMembers = useMemo(() => {
    ...
    // If we got data from the database function, it's already filtered
    // Otherwise, apply period-based filtering
    try {
      const currentDateRange = generateDateRange(currentMonthIndex);
      
      // Check if data looks pre-filtered (from database function)
      const isPreFiltered = rawStaffData.length > 0 && 
        Object.prototype.hasOwnProperty.call(rawStaffData[0], 'name') && 
        !Object.prototype.hasOwnProperty.call(rawStaffData[0], 'start_period');

      if (isPreFiltered) {
        console.log(
          `📋 Using pre-filtered staff from normalized architecture: ${transformedStaff.length} active staff for period ${currentMonthIndex}`,
        );
  ...
  const saveStaffMutation = useMutation({
    mutationFn: async ({ staffMembers: newStaffMembers, operation, updateAssignments = true }) => {
      ...
      // Step 1: Save staff to app_staff table
