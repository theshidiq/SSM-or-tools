...
export const useConflictResolution = (options = {}) => {
  ...
  const resolveConflict = useCallback(
    async (localData, remoteData, metadata = {}) => {
      ...
      try {
        ...
        console.log(
          `🔧 Conflict resolution ${resolution.resolved ? "completed" : "requires user input"}:`,
          {
            strategy: resolution.strategy,
        ...
        if (conflictQueueRef.current.length > 0) {
          ...
          setTimeout(() => {
            resolveConflict(nextConflict.localData, nextConflict.remoteData, {
              ...nextConflict.metadata,
              fromQueue: true,
            });
  ...
  const resolveUserConflict = useCallback(
  ...
  const applyManualResolutions = useCallback(
    async (localData, remoteData, resolutions) => {
      ...
      for (const [conflictKey, resolution] of Object.entries(resolutions)) {
        ...
        if (type === "schedule") {
          ...
          switch (resolution.choice) {
            case "local":
              result.schedule_data[staffId][dateKey] =
                localData.schedule_data[staffId]?.[dateKey];
              break;
            case "remote":
              result.schedule_data[staffId][dateKey] =
                remoteData.schedule_data[staffId]?.[dateKey];
              break;
            case "custom":
          ...
          const staffIndex = result._staff_members.findIndex(
            (s) => s.id === staffId,
          );
          ...
          if (staffIndex !== -1) {
            switch (resolution.choice) {
              case "local":
                const localStaff = localData._staff_members.find(
                  (s) => s.id === staffId,
                );
                if (localStaff) {
