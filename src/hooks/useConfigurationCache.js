...
export const useConfigurationCache = () => {
  ...
  const refreshCache = useCallback(async (settingType = null) => {
    try {
      ...
      console.log(`🔄 Refreshing configuration cache${settingType ? ` for ${settingType}` : ''}...`);
...
export const useSettingsCache = () => {
  ...
  const onSettingSaved = useCallback(async (settingType) => {
  ...
  return {
    ...
    onSettingsBulkSaved,
    cacheStatus,
    isRefreshing: cacheStatus === 'refreshing' || cacheStatus === 'force-refreshing',
  };
};
