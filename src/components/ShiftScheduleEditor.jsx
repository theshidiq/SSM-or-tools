...
const ShiftScheduleEditor = () => {
  ...
  // UUID mapping for legacy staff IDs
  const legacyStaffIdMap = {
  ...
  // Function to migrate legacy schedule data from string IDs to UUIDs
  const migrateScheduleData = (scheduleData) => {
    ...
    // Copy UUID-based entries and _staff_members as-is
    Object.keys(scheduleData).forEach(key => {
      if (key.startsWith('01934d2c-') || key === '_staff_members') {
        migratedSchedule[key] = scheduleData[key];
      } else if (legacyStaffIdMap[key]) {
        // Migrate legacy string IDs to UUIDs
        migratedSchedule[legacyStaffIdMap[key]] = scheduleData[key];
      }
    });
  ...
  const migrateStaffMembers = (staffList) => {
    ...
    return staffList.map(staff => {
      ...
      if (typeof staff === 'string') {
        const staffKey = staff.toLowerCase();
        const uuid = legacyStaffIdMap[staffKey];
        const defaults = defaultStaffProperties[staff];
        
        return {
          id: uuid || `temp-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
          name: staff,
          position: defaults?.position || 'Staff',
          color: defaults?.color || 'position-server',
          status: defaults?.status || '派遣',
          startPeriod: defaults?.startPeriod || { year: 2018, month: 4, day: 1 },
          endPeriod: defaults?.endPeriod || null
        };
      }
      ...
      return {
        ...staff,
        id: staff.id || `temp-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
  ...
  useEffect(() => {
    ...
    if (savedSchedule) {
      ...
      if (supabaseScheduleData && supabaseScheduleData.schedule_data) {
        ...
        // Migrate legacy schedule data to UUID format
        const migratedScheduleData = migrateScheduleData(actualScheduleData);
