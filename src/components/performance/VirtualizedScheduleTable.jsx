...
const VirtualizedScheduleTable = React.memo(
  ({
    ...
    useEffect(() => {
      const calculateDimensions = () => {
        ...
        const maxColumns = Math.min(
          maxVisibleColumns,
          Math.floor(
            (availableWidth - stickyColumns * columnWidth) / columnWidth,
          ) + stickyColumns,
        );
        ...
        const height = Math.min(
          availableHeight,
          maxRows * rowHeight + headerHeight,
        );
    ...
    return (
      <div
        className={`virtualized-schedule-table ${className}`}
      ...
      >
        ...
        {process.env.NODE_ENV === "development" && (
          <div className="absolute bottom-2 right-2 text-xs text-gray-500 bg-white px-2 py-1 rounded shadow">
            ...
            totalHeight > dimensions.height - headerHeight
              ? " (virtualized)"
              : " (all visible)"}
          </div>
        )}
...
const DateHeaderCell = React.memo(
  ({ style, date, dayName, dayNumber, dateIndex }) => {
    ...
    return (
      ...
      >
        ...
        <div
          className={`text-lg ${isToday ? "text-yellow-800" : isWeekend ? "text-red-600" : "text-gray-900"}`}
        >
          {dayNumber}
        </div>
      </div>
  ...
  ({
    ...
    return (
      <div
        ...
        title={
          isLocked
            ? `編集中: ${lockInfo?.userName || "Unknown User"}`
            : undefined
        }
      >
        {/* Cursor indicators */}
