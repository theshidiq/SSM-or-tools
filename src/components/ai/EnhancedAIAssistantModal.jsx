            ...
            <div className="flex items-center gap-2">
              ...
              {performanceMetrics.metrics && (
                <div className="flex items-center gap-1 text-xs bg-white/20 rounded px-2 py-1">
                  ...
                  <span>
                    {Math.round(
                      performanceMetrics.metrics.uiResponsiveness
                        ?.averageFrameTime || 16,
                    )}
                    ms
                  </span>
                  ...
                  {progressState.stats?.totalWork && (
                    <>
                      <div>
                        ...
                        <span className="ml-2 font-medium">
                          {progressState.stats.completedWork || 0} /{" "}
                          {progressState.stats.totalWork}
                        </span>
                      </div>
                      <div>
                        ...
                        <span className="ml-2 font-medium">
                          {Math.round(
                            (progressState.stats.processingTime || 0) / 1000,
                          )}
                          s
                        </span>
                      </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <span className="ml-2 font-medium">
                      {Math.round(
                        performanceMetrics.metrics.averageProcessingTime || 0,
                      )}
                      ms
                    </span>
                  </div>
                  <div>
                    ...
                    <span className="ml-2 font-medium">
                      {performanceMetrics.metrics.totalProcessingJobs > 0
                        ? Math.round(
                            (performanceMetrics.metrics.successfulJobs /
                              performanceMetrics.metrics.totalProcessingJobs) *
                              100,
                          )
                        : 0}
                      %
            ...
            {systemStatus && systemStatus.type === "enhanced" && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                ...
                <div className="grid grid-cols-2 gap-4 text-xs text-blue-800">
                  <div>初期化: {systemStatus.initialized ? "✓" : "⏳"}</div>
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      ...
                      <div className="bg-gray-50 rounded-lg p-3 text-center">
                        ...
                        <div className="text-2xl font-bold text-blue-600">
                          {Math.round(
                            (results.data.processingTime || 0) / 1000,
                          )}
                          s
                        </div>
                      </div>
                    ...
                    {results.data.performanceStats && (
                      <div className="bg-purple-50 border border-purple-200 rounded-lg p-3">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs text-purple-800">
                          <div>
                            メモリ効率:{" "}
                            {results.data.performanceStats.memoryEfficiency ||
                              0}
                            %
                          </div>
                          <div>
                            ストリーミング:{" "}
                            {results.data.performanceStats.streamingEnabled
                              ? "有効"
                              : "無効"}
                          </div>
                          <div>
                            Worker使用:{" "}
                            {results.data.performanceStats.workerUsed
                              ? "有効"
                              : "無効"}
                          </div>
                          <div>
                            キャッシュヒット:{" "}
                            {results.data.performanceStats.cacheHitRate || 0}%
                          </div>
                        </div>
                      </div>
                    )}
