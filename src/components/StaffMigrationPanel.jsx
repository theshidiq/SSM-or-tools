...
const StaffMigrationPanel = () => {
    ...
    try {
      ...
      if (localStorageCheck.hasData) {
        ...
        if (extractedData) {
          ...
          Object.entries(extractedData.periodBased).forEach(
            ([period, periodData]) => {
              const transformedStaff = transformStaffDataForDatabase(
                periodData.data,
              );
              previewData = previewData.concat(
          ...
          Object.entries(extractedData.injected).forEach(
            ([period, injectedData]) => {
              const transformedStaff =
                transformStaffDataForDatabase(injectedData);
              previewData = previewData.concat(
                transformedStaff.map((staff) => ({
                  ...staff,
  ...
  const handleRollback = async () => {
    ...
    if (
      !confirm(
        "Are you sure you want to rollback the migration? This will restore localStorage data and you may lose any new data added since migration.",
      )
    ) {
      return;
    }
  ...
  const getStatusText = () => {
    switch (migrationState.status) {
      case "analyzing":
        return "Analyzing localStorage data...";
      case "migrating":
        return "Migrating staff data...";
      case "completed":
        return migrationState.hasData
          ? "Migration completed successfully!"
          : "No migration needed - no localStorage data found";
      case "error":
        return `Error: ${migrationState.error}`;
      default:
        return migrationState.hasData
          ? "Ready to migrate staff data"
          : "No staff data found in localStorage";
    }
  };
    ...
    <div className="bg-white p-6 rounded-lg shadow-lg border-2 border-blue-200">
      <div className="flex items-center mb-4">
        ...
        <h3 className="text-lg font-semibold text-gray-800">
          Phase 4: Staff Data Migration
        </h3>
      </div>

      {/* Status Display */}
      <div
        className={`mb-4 p-3 rounded-md bg-gray-50 ${getStatusColor(migrationState.status)}`}
      >
      ...
      {migrationState.hasData && (
        <div className="mb-4 p-3 bg-blue-50 rounded-md">
          ...
          <p className="text-sm text-blue-600 mt-2">
            {migrationState.preview.length} unique staff members ready for
            migration
          </p>
        </div>
      )}
      ...
      {migrationState.preview.length > 0 && (
        <div className="mb-4">
          ...
          >
            ...
            <span className="ml-1">
              Preview staff data ({migrationState.preview.length} members)
            </span>
          </button>
          ...
          {showAdvancedOptions && (
            <div className="space-y-2 p-3 bg-gray-50 rounded-md">
                  ...
                  onChange={(e) =>
                    setMigrationOptions((prev) => ({
                      ...
                      updateDuplicates: e.target.checked
                        ? false
                        : prev.updateDuplicates,
                    }))
                  }
                  className="mr-2"
                />
                <span className="text-sm">
                  Skip duplicate staff members (recommended)
                </span>
              </label>
              ...
              <label className="flex items-center">
                ...
                />
                <span className="text-sm">
                  Update duplicate staff members instead of skipping
                </span>
              </label>
        ...
        <div className="mt-4 p-3 bg-gray-50 rounded-md">
          ...
          <div className="text-sm space-y-1">
            ...
            <p>
              ...
              {migrationState.result.success ? "✅ Success" : "❌ Failed"}
            </p>
            <p>
            ...
            {migrationState.result.migrated !== undefined && (
              <p>
                ...
                {migrationState.result.migrated} staff members
              </p>
            )}
            {migrationState.result.duplicates !== undefined &&
              migrationState.result.duplicates > 0 && (
                <p>
                  <span className="font-medium">Duplicates skipped:</span>{" "}
                  {migrationState.result.duplicates}
                </p>
              )}
