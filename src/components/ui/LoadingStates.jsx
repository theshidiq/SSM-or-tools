...
export const AILoadingSpinner = ({ message = 'Loading AI features...' }) => {
  return (
    <div className="flex flex-col items-center justify-center py-8 space-y-4">
      ...
      <div className="text-gray-400 text-xs">This may take a moment for first-time loading...</div>
    </div>
  );
};
...
export const FeatureLoadingProgress = ({ features = [], currentFeature = '', progress = 0 }) => {
  return (
    <div className="space-y-4 p-6 bg-white border rounded-lg shadow-sm">
      <div className="text-center">
        <h3 className="text-lg font-medium text-gray-900 mb-2">Loading AI Features</h3>
          ...
          {features.map((feature, index) => (
            <div key={index} className="flex items-center space-x-2 text-sm">
              <div className={`w-2 h-2 rounded-full ${
                feature.loaded ? 'bg-green-500' : 
                feature.loading ? 'bg-blue-500 animate-pulse' : 'bg-gray-300'
              }`}></div>
              <span className={feature.loaded ? 'text-gray-600' : 'text-gray-400'}>
                {feature.name}
              </span>
            </div>
          ))}
