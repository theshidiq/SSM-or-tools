...
const ShiftScheduleEditorRealtime = ({
  ...
  useEffect(() => {
    if (!periodsLoading && realtimePeriods.length > 0) {
      try {
        ...
        console.log(
          `📅 Set initial period index to ${correctIndex} (${realtimePeriods[correctIndex]?.label})`,
        );
      } catch (error) {
    ...
    if (!periodsLoading && realtimePeriods.length > 0) {
      ...
      if (pendingNavigationToPeriod) {
        ...
        if (targetPeriodIndex >= 0) {
          ...
          console.log(
            `📅 Navigated to newly added period: ${realtimePeriods[targetPeriodIndex]?.label} (index ${targetPeriodIndex})`,
          );
          return;
        } else {
          // Period not found yet, keep waiting for real-time sync with timeout fallback
          console.log(
            `⏳ Waiting for newly added period '${pendingNavigationToPeriod.label}' to sync from database...`,
          );
          console.log(
            `🔍 Currently have ${realtimePeriods.length} periods, looking for: ${pendingNavigationToPeriod.start.toISOString().split("T")[0]} to ${pendingNavigationToPeriod.end.toISOString().split("T")[0]}`,
          );

          // Set a timeout to fallback to last period if sync doesn't complete
          navigationTimeoutRef.current = setTimeout(async () => {
            console.warn(
              `⚠️ Timeout waiting for period '${pendingNavigationToPeriod.label}' - trying force refresh before fallback`,
            );
            ...
            try {
              ...
              const targetPeriodIndexAfterRefresh = realtimePeriods.findIndex(
                (period) => {
                  return (
                    period.label === pendingNavigationToPeriod.label &&
                    period.start.toISOString().split("T")[0] ===
                      pendingNavigationToPeriod.start
                        .toISOString()
                        .split("T")[0] &&
                    period.end.toISOString().split("T")[0] ===
                      pendingNavigationToPeriod.end.toISOString().split("T")[0]
              ...
              if (targetPeriodIndexAfterRefresh >= 0) {
                ...
                setPendingNavigationToPeriod(null);
                console.log(
                  `✅ Found period after force refresh: ${realtimePeriods[targetPeriodIndexAfterRefresh]?.label} (index ${targetPeriodIndexAfterRefresh})`,
                );
                return;
              }
            } catch (error) {
            ...
            setPendingNavigationToPeriod(null);
            console.log(
              `📅 Fallback navigation to index ${fallbackIndex} (${realtimePeriods[fallbackIndex]?.label})`,
            );
      ...
      if (
        ...
        const newIndex =
          realtimePeriods.length === 0
            ...
            : Math.max(
                0,
                Math.min(currentMonthIndex, realtimePeriods.length - 1),
              );
        if (newIndex !== currentMonthIndex) {
          setCurrentMonthIndex(newIndex);
          console.log(
            `🔄 Period index out of bounds (${currentMonthIndex}), adjusted to ${newIndex}${realtimePeriods[newIndex] ? ` (${realtimePeriods[newIndex].label})` : " (no periods available)"}`,
          );
        }
      }
  ...
  useEffect(() => {
    console.log(
      `🔍 ShiftScheduleEditor state: currentMonthIndex=${currentMonthIndex}, realtimePeriods.length=${realtimePeriods.length}`,
    );
  }, [currentMonthIndex, realtimePeriods.length]);
  ...
  const realtimeStatus = useMemo(() => {
    if (!isConnected)
      return {
        status: "disconnected",
        message: "Offline - Auto-reconnecting...",
      };
  ...
  const handleDeletePeriod = useCallback(() => {
    ...
    if (!realtimePeriods || realtimePeriods.length === 0) {
      setDeleteModal({
        ...
        message:
          "No periods available to delete. Please create a period first using the 'Add Table' button.",
      });
      return;
    }
    ...
    const periodLabel =
      realtimePeriods[indexToCheck]?.label || "current period";

    // Allow deletion of all periods now - user can recreate with "Add Table"
    const warningMessage =
      realtimePeriods.length <= 1
  ...
  const confirmDeletePeriod = useCallback(async () => {
    ...
    if (indexToDelete < 0 || indexToDelete >= realtimePeriods.length) {
      ...
      indexToDelete = Math.max(
        0,
        Math.min(currentMonthIndex, realtimePeriods.length - 1),
      );
      console.warn(
        `⚠️ Adjusted deletion index from ${currentMonthIndex} to ${indexToDelete} (available: 0-${realtimePeriods.length - 1})`,
      );
    ...
    const periodToDeleteLabel =
      realtimePeriods[indexToDelete]?.label || `period ${indexToDelete}`;
    ...
    try {
      ...
      await forcePeriodsRefresh();
      console.log(
        `🔄 Forced periods refresh - now have ${realtimePeriods.length} periods`,
      );
      ...
      if (deletionResult.isEmpty) {
        ...
        if (newCurrentIndex >= updatedPeriodCount) {
          ...
          console.log(
            `⚠️ Adjusted navigation index from ${deletionResult.suggestedNavigationIndex} to ${newCurrentIndex} (max available: ${updatedPeriodCount - 1})`,
          );
        }

        console.log(
          `🔄 Navigating to period ${newCurrentIndex} after deletion (${updatedPeriodCount} periods available)`,
        );
      ...
      const successMessage = deletionResult.isEmpty
        ...
        : `The period "${deletionResult.deletedPeriod.label}" has been completely removed from the system.${
            deletionResult.suggestedNavigationIndex >= 0 &&
            deletionResult.suggestedNavigationIndex < realtimePeriods.length &&
        ...
        <NavigationToolbar
          ...
          handleAddTable={async () => {
            ...
            try {
              ...
              if (!lastPeriod) {
                console.error(
                  "Cannot add next period - no valid last period found",
                );
                return;
              }
              ...
              console.log(
                `🔄 Adding new period: ${expectedLabel} (${nextStartDate.toISOString().split("T")[0]} to ${nextEndDate.toISOString().split("T")[0]})`,
              );
              ...
              if (typeof newPeriodIndex === "number" && newPeriodIndex >= 0) {
                ...
                const existingPeriodIndex = realtimePeriods.findIndex(
                  (period) => {
                    return (
                      period.label === expectedLabel &&
                      period.start.toISOString().split("T")[0] ===
                        expectedPeriod.start.toISOString().split("T")[0] &&
                      period.end.toISOString().split("T")[0] ===
                        expectedPeriod.end.toISOString().split("T")[0]
                    );
                  },
                );
                ...
                if (existingPeriodIndex >= 0) {
                  ...
                  console.log(
                    `📅 Navigated immediately to existing period: ${expectedLabel} (index ${existingPeriodIndex})`,
                  );
                } else {
                  ...
                  console.log(
                    `📅 Immediately adding and navigating to new period: ${expectedLabel}`,
                  );
                  ...
                  try {
                    ...
                    const newPeriodIndex = realtimePeriods.findIndex(
                      (period) =>
                    ...
                    if (newPeriodIndex >= 0) {
                      ...
                      console.log(
                        `✅ Successfully navigated to new period: ${expectedLabel} (index ${newPeriodIndex})`,
                      );
                    } else {
                      console.log(
                        `⚠️ Period not found after refresh, falling back to pending navigation`,
                      );
                      setPendingNavigationToPeriod(expectedPeriod);
                    }
                  } catch (error) {
                    console.error(
                      "Failed to force refresh after adding period:",
                      error,
                    );
                    setPendingNavigationToPeriod(expectedPeriod);
                  }
