...
const ConnectionStatusBanner = ({
  ...
  let message = "";
  const actionText = "Retry Connection";
  ...
  if (!database) {
    ...
    message =
      "Unable to connect to the database. Settings will use local storage as fallback.";
  } else if (!configuration) {
    bannerType = "warning";
    icon = AlertCircle;
    title = "Configuration Tables Unavailable";
    message =
      "Database is connected but configuration tables are not accessible. Settings will be saved locally.";
  }
  ...
  return (
    <div
      className={`mx-6 mt-4 p-4 rounded-lg border flex items-start gap-3 ${bannerClasses[bannerType]}`}
    >
      <IconComponent
        size={20}
        className={`flex-shrink-0 mt-0.5 ${iconClasses[bannerType]}`}
      />
      <div className="flex-1">
        ...
        {bannerType === "warning" && (
          <p className="text-xs mt-2 opacity-75">
            Your settings will still be saved and work normally, but may not be
            synchronized across devices until database access is restored.
          </p>
        )}
      </div>
      {onRetryConnection && (
