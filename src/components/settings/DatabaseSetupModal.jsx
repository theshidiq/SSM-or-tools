...
const DatabaseSetupModal = ({ isOpen, onClose, onComplete }) => {
  const [automatedService] = useState(
    () => new AutomatedDatabaseSetupService(),
  );
  ...
  useEffect(() => {
    if (isOpen) {
      const service =
        setupMode === "automated" ? automatedService : fallbackService;
      const steps =
        setupMode === "automated"
      ...
      if (result.success) {
        ...
        if (setupMode === "automated" && result.fallbackRequired) {
          ...
          if (setupMode === "manual") {
            ...
            if (
              progressInfo.generatedSchema &&
              progressInfo.missingTables?.length > 0
            ) {
  ...
  const executeSetup = async (mode = setupMode) => {
    try {
      ...
      await service.executeSetup(handleProgress, handleError, handleComplete);
    } catch (error) {
  ...
  const switchToManualSetup = () => {
    ...
    if (
      automatedService &&
      typeof automatedService.generateFallbackSQL === "function"
    ) {
  ...
  const handleClose = () => {
    if (setupState === "running") {
      if (
        window.confirm(
          "Database setup is in progress. Are you sure you want to cancel?",
        )
      ) {
        onClose();
      }
        ...
        <div className="flex-1 overflow-hidden flex">
                ...
                {setupSteps.map((step, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div
                        className={`font-medium text-sm ${getStepStatusColor(step)}`}
                      >
                        {index + 1}. {step.name}
                      </div>
          ...
          <div className="flex-1 p-6">
            ...
            <div className="mb-6">
                ...
                !automationFailed && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-600 mb-6 max-w-md mx-auto">
                      RPC-FREE one-click solution! All tables, functions, and
                      sample data will be created automatically using direct SQL
                      execution. No RPC setup or manual copy-pasting required.
                    </p>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="text-sm text-green-800">
                          ...
                          <ul className="space-y-1 text-green-700">
                            <li>
                              • NO RPC functions required - works immediately
                            </li>
                            <li>• Direct SQL execution via supabase.sql()</li>
                            <li>
                              • Smart table detection (skips existing tables)
                            </li>
              ...
              {setupState === "idle" &&
                (setupMode === "manual" || automationFailed) && (
                  <div className="text-center py-8">
                    <Database
                      size={48}
                      className="mx-auto text-blue-500 mb-4"
                      ...
                      <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-6">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="text-sm text-orange-800">
                            <p className="font-medium mb-2">
                              RPC-Free Automated Setup Failed
                            </p>
                            <p className="mb-2">
                              The RPC-free automated setup couldn't complete due
                              to database permissions or connection issues.
                    ...
                    <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
                      <div className="text-sm text-green-800">
                        <p className="font-medium">
                          ✅{" "}
                          {setupMode === "automated"
                            ? setupResult.chunksCompleted
                            : setupResult.stepsCompleted}{" "}
                          {setupMode === "automated" ? "chunks" : "steps"}{" "}
                          completed successfully
                        </p>
                        {setupResult.tablesCreated &&
                          setupResult.tablesCreated.length > 0 && (
                            <p className="mt-1">
                              🗂️ Created {setupResult.tablesCreated.length} new
                              tables
                            </p>
                          )}
                        {setupResult.tablesExisted &&
                          setupResult.tablesExisted.length > 0 && (
                            <p className="mt-1">
                              ⏭️ Skipped {setupResult.tablesExisted.length}{" "}
                              existing tables
                            </p>
                          )}
                        {errors.length > 0 && (
                          <p className="mt-1">
                            ⚠️ {errors.length} non-critical warnings (see
                            details below)
                          </p>
                        )}
                      </div>
                      ...
                      {(schemaText || setupMode === "manual") && (
                        <button
                          onClick={() => {
                            if (!schemaText) {
                              const schema =
                                fallbackService.getGeneratedSchema();
                              setSchemaText(schema);
                            }
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="text-sm text-yellow-800">
                        ...
                        <ol className="list-decimal list-inside space-y-1 text-yellow-700">
                          ...
                          <li>
                            Return here and click "Validate Setup" to continue
                          </li>
                        </ol>
                      </div>
                    </div>
                ...
                {errors.length > 0 && (
                  <div className="mb-6">
                    <h5 className="font-medium text-gray-700 mb-2">
                      {setupState === "failed" ? "Errors" : "Warnings"} (
                      {errors.length})
                    </h5>
