...
export class HighAccuracyMLScheduler {
    ...
    try {
      ...
      if (this.restaurantId) {
        try {
          ...
          console.warn(
            "⚠️ Configuration service integration failed, using defaults:",
            error,
          );
          this.configService = null;
        }
      ...
      console.log(
        `🎯 Target accuracy: ${(this.config.targetAccuracy * 100).toFixed(1)}%`,
      );
  ...
  async trainModel(staffMembers = null, options = {}) {
    ...
    try {
      ...
      console.log(
        `📊 Training data analysis: ${dataAnalysis.quality}% quality, ${dataAnalysis.samples} samples`,
      );
      ...
      if (this.config.enableEnsemble) {
        ...
        ensembleResult = await this.ensembleScheduler.trainEnsemble(
          staffMembers,
          {
            ...options,
            targetAccuracy: this.config.targetAccuracy,
        ...
        if (ensembleResult.success) {
          console.log(
            `✅ Ensemble training completed: ${(ensembleResult.accuracy * 100).toFixed(2)}% accuracy`,
          );
          this.performance.currentAccuracy = ensembleResult.accuracy;
        } else {
          console.warn(
            "⚠️ Ensemble training failed, falling back to standard training",
          );
        }
      }
      ...
      if (this.config.enableFallback) {
        ...
        fallbackResult = await this.fallbackScheduler.trainModel(
          staffMembers,
          options,
        );
        ...
        if (fallbackResult.success) {
          console.log(
            `✅ Fallback training completed: ${(fallbackResult.accuracy * 100).toFixed(2)}% accuracy`,
          );
        }
      }

      // Select best performing model
      const bestModel = this.selectBestTrainedModel(
        ensembleResult,
        fallbackResult,
      );
      ...
      console.log(
        `🎉 High-accuracy training completed in ${this.performance.lastTrainingTime}ms`,
      );
      console.log(
        `🎯 Final accuracy: ${(this.performance.currentAccuracy * 100).toFixed(2)}%`,
      );
  ...
  async predict(inputParams) {
    ...
    try {
      ...
      switch (this.currentModel) {
        case "ensemble": {
          const ensembleResult = await this.ensembleScheduler.predict([
            features,
          ]);
        ...
        case "fallback": {
          const fallbackResult = await this.fallbackScheduler.predict([
            features,
          ]);
      ...
      // Quality check
      const qualityCheck = this.qualityController.validatePrediction(
        prediction,
        confidence,
        inputParams,
      );
      ...
      const result = {
        ...
        features: this.config.enableAdvancedFeatures
          ? features.slice(0, 10)
          : null, // Limited for debugging
      };
  ...
  getStatus() {
    return {
      ...
      systemHealth: {
        ensembleAvailable:
          this.config.enableEnsemble &&
          this.ensembleScheduler.getStatus().initialized,
        fallbackAvailable:
          this.config.enableFallback &&
          this.fallbackScheduler.getStatus?.().initialized,
  ...
  async forceRetrain(staffMembers = null, options = {}) {
    ...
    return await this.trainModel(staffMembers, {
      ...options,
      forceRetrain: true,
    });
  }
  ...
  async analyzeTrainingData(staffMembers) {
    try {
      ...
      const totalSamples = Object.values(dataResult.allHistoricalData).reduce(
        (sum, period) =>
          sum + (period.dateRange?.length || 0) * (staffMembers?.length || 0),
        0,
      );
  ...
  updatePerformanceMetrics(ensembleResult, fallbackResult) {
    if (
      ...
      ensembleResult.accuracy > this.performance.currentAccuracy
    ) {
      this.performance.currentAccuracy = ensembleResult.accuracy;
    } else if (
      fallbackResult?.success &&
      fallbackResult.accuracy > this.performance.currentAccuracy
    ) {
