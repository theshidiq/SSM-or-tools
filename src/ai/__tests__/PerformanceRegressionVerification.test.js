...
describe('Performance Regression Verification', () => {
  ...
  const mockStaffMembers = [
    ...
    { id: 'staff3', name: '山田次郎', status: '社員', position: 'マネージャー' },
  ...
  test('should handle 270 predictions without timeout (original issue)', async () => {
    ...
    console.log(`🎯 Testing 270 predictions (${numStaff} staff × ${numDates} dates)`);
    ...
    console.log(`  Average per prediction: ${avgTimePerPrediction.toFixed(1)}ms`);
    ...
    const under50msCount = results.filter((_, i) => 
      avgTimePerPrediction < 50 // Simplified check
    ).length;
    
    console.log(`📊 Success metrics:`);
    console.log(`  Completed: ${results.length}/${predictions.length} (100%)`);
    console.log(`  Under 50ms target: ${(under50msCount / results.length * 100).toFixed(1)}%`);
  ...
  test('should maintain performance under realistic load patterns', async () => {
    ...
    for (const test of loadTests) {
      console.log(`🔄 Testing ${test.name}: ${test.staff} staff × ${test.dates} dates`);
      ...
      console.log(`  ${test.name}: ${results.length} predictions in ${totalTime.toFixed(0)}ms (${avgTime.toFixed(1)}ms avg)`);
...
describe('Regression Prevention', () => {
  test('should prevent future performance regressions', () => {
    ...
    console.log(`  Single prediction: <${performanceTargets.singlePrediction}ms`);
    console.log(`  Batch average: <${performanceTargets.batchAverage}ms per prediction`);
    console.log(`  Timeout threshold: <${performanceTargets.timeoutThreshold}ms`);
    console.log(`  Max concurrent: ${performanceTargets.maxPredictions} predictions`);
