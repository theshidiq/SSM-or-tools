...
describe("Phase 6: Edge Case & Error Handling Tests", () => {
  ...
  describe("Memory Constraint Handling", () => {
    test("should handle low memory conditions gracefully", async () => {
      ...
      try {
        ...
        if (!memoryConstraint.checkMemoryLimit()) {
          ...
          fallbackActivated =
            result.metadata?.method?.includes("rule") ||
    ...
    test("should cleanup tensors properly to prevent memory leaks", async () => {
      ...
      console.log(
        `  ✅ Memory increase: ${Math.round(memoryIncrease / 1024 / 1024)}MB total`,
      );
      console.log(
        `  ✅ Per iteration: ${Math.round(memoryIncreasePerIteration / 1024 / 1024)}MB`,
      );
    });
  });
  ...
  describe("Insufficient Data Handling", () => {
    test("should handle minimal historical data gracefully", async () => {
      ...
      gracefulDegradation =
        scheduleGenerated && (ruleBased || result.metadata?.mlConfidence < 0.5);
      ...
      console.log(
        `  ✅ Handled minimal data (${minimalData.dataAmount * 100}% of normal)`,
      );
    ...
    test("should handle empty historical data", async () => {
      ...
      try {
        ...
        if (fallbackGenerated) {
          ...
          const totalCells =
            Object.keys(result.schedule).length * emptyData.dateRange.length;
          const filledCells = Object.values(result.schedule).reduce(
            (count, staffSched) => {
              return (
                count +
                Object.values(staffSched).filter((shift) => shift !== "").length
              );
            },
            0,
          );
      ...
      console.log(
        `  ✅ Empty data handled with ${(scheduleQuality * 100).toFixed(1)}% schedule quality`,
      );
    });
  });
    ...
    test("should validate and handle malformed input data", async () => {
      ...
      const malformedInputs = [
        ...
        { name: "Empty date range", data: { dateRange: [] } },
        {
          ...
          data: { dateRange: ["invalid_date", null] },
        },
        {
          name: "Corrupted schedule",
          data: { scheduleData: { staff1: "not_an_object" } },
        },
      ];
        ...
        try {
          const testData = {
            ...
            staffMembers: Array.isArray(input.data.staffMembers)
              ? input.data.staffMembers
              : createTestStaffMembers(3),
            dateRange: Array.isArray(input.data.dateRange)
              ? input.data.dateRange
              : generateLargeDateRange(7),
          };
  ...
  describe("Extreme Scale Testing", () => {
    test("should handle very large staff and date ranges", async () => {
      ...
      reportGenerator.addTestResult("Extreme Scale Testing", {
        ...
        maxProcessingTime: Math.max(
          ...scaleResults.map((r) => r.processingTime),
        ),
    ...
    test("should maintain responsiveness during intensive operations", async () => {
      console.log(
        "⚡ Testing system responsiveness during intensive operations...",
      );
      ...
      const avgPingTime =
        responsivenessPings.length > 0
          ? responsivenessPings.reduce((sum, ping) => sum + ping, 0) /
            responsivenessPings.length
          : 0;
      ...
      console.log(
        `  ✅ Responsiveness maintained: ${responsivenessMaintained}`,
      );
    });
  });
