...
describe("Phase 2: Core Prediction Engine", () => {
  ...
  describe("GeneticAlgorithm", () => {
    ...
    test("should improve fitness over generations", async () => {
      ...
      expect(result.evolutionHistory.fitnessHistory).toHaveLength(
        result.generations,
      );
  ...
  describe("ConflictResolver", () => {
    ...
    test("should handle insufficient coverage", async () => {
      ...
      const workingStaff = Object.keys(result.schedule).filter(
        (staffId) => result.schedule[staffId]["2025-01-21"] !== "×",
      );
  ...
  describe("OptimizationEngine", () => {
    ...
    test("should generate optimization recommendations", async () => {
      const recommendations =
        await optimizationEngine.generateOptimizationRecommendations(
          mockSchedule,
