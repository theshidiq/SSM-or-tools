...
describe('Phase 6: Performance & Memory Testing', () => {
  ...
  describe('Training Performance Testing', () => {
    test('should meet training time targets', async () => {
      ...
      const avgTrainingTime = trainingResults.reduce((sum, r) => sum + r.trainingTime, 0) / trainingResults.length;
      const maxMemoryUsed = Math.max(...trainingResults.map(r => r.memoryUsed));
      ...
      reportGenerator.addTestResult('Training Performance', {
        ...
        avgTrainingTime,
        maxMemoryUsed,
        allMeetTargets: trainingResults.every(r => r.meetsTimeTarget && r.meetsMemoryTarget)
      });
    ...
    test('should scale training performance predictably', async () => {
      ...
      for (const factor of scalingFactors) {
        ...
        console.log(`    ${factor}x data: ${(trainingTime / 1000).toFixed(1)}s (${efficiency.toFixed(2)}x efficiency)`);
      }
      
      // Efficiency should not degrade too much (>50% efficiency for reasonable scaling)
      const minEfficiency = Math.min(...scalingResults.slice(1).map(r => r.efficiency));
      ...
      console.log(`  ✅ Minimum scaling efficiency: ${(minEfficiency * 100).toFixed(1)}%`);
    });
  });
  ...
  describe('Prediction Performance Testing', () => {
    test('should meet prediction time targets', async () => {
      ...
      for (const scenario of predictionScenarios) {
        ...
        const timeLimit = scenario.staffCount > 30 ? 
      ...
      const avgPredictionTime = predictionResults.reduce((sum, r) => sum + r.predictionTime, 0) / predictionResults.length;
      
      reportGenerator.addTestResult('Prediction Performance', {
        success: true,
        predictionResults,
        avgPredictionTime,
        allMeetTargets: predictionResults.every(r => r.meetsTimeTarget && r.meetsMemoryTarget)
      });
      
      console.log(`  ✅ Average prediction time: ${(avgPredictionTime / 1000).toFixed(2)}s`);
    });
    ...
    test('should achieve target throughput', async () => {
      ...
      while (Date.now() - startTime < testDuration) {
        const promise = hybridPredictor.predictSchedule(
          { scheduleData: testData.scheduleData },
          testData.staffMembers,
          testData.dateRange
        ).then(result => {
      ...
      const avgProcessingTime = throughputTests.length > 0 ?
        throughputTests.reduce((sum, test) => sum + test.processingTime, 0) / throughputTests.length : 0;
      ...
      console.log(`  ✅ Throughput: ${throughput.toFixed(1)} predictions/second`);
  ...
  describe('Memory Management Testing', () => {
    test('should prevent memory leaks during repeated operations', async () => {
      ...
      const memoryLeakDetected = memoryIncreasePerIteration > PERFORMANCE_TARGETS.MEMORY_LEAK_THRESHOLD / iterations;
      ...
      reportGenerator.addTestResult('Memory Leak Prevention', {
        ...
        memoryIncreasePerIteration: Math.round(memoryIncreasePerIteration / 1024),
      ...
      console.log(`  ✅ Per iteration: ${Math.round(memoryIncreasePerIteration / 1024)}KB`);
      console.log(`  ✅ Memory trend: ${memoryTrend.slope > 0 ? 'increasing' : 'stable'}`);
    });
    ...
    test('should handle memory pressure gracefully', async () => {
      ...
      try {
        ...
        for (let i = 0; i < 50; i++) { // Try to allocate 50MB
          memoryPressureArrays.push(new Array(arraySize).fill(0));
          
          if (getMemoryUsage() > 150 * 1024 * 1024) { // 150MB threshold
        ...
        console.log(`  Memory after pressure: ${Math.round(getMemoryUsage() / 1024 / 1024)}MB`);
        ...
        const gracefulHandling = result.success && (
        ...
        console.log(`  ✅ Operation succeeded under memory pressure: ${result.success}`);
    ...
    test('should cleanup TensorFlow tensors properly', async () => {
      ...
      const tensorLeak = finalTensorCount > initialTensorCount + (tensorOperations * 2); // Allow some overhead
  ...
  describe('Load Testing', () => {
    test('should maintain performance under concurrent load', async () => {
      ...
      const sortedTimes = successfulOperations.map(r => r.responseTime).sort((a, b) => a - b);
      const p99Index = Math.floor(sortedTimes.length * 0.99);
      const p99ResponseTime = sortedTimes[p99Index] || maxResponseTime;
      
      expect(successRate).toBeGreaterThan(0.95); // 95% success rate
      expect(avgResponseTime).toBeLessThan(PERFORMANCE_TARGETS.PREDICTION_TIME * 1.5); // Allow 50% longer under load
      expect(p99ResponseTime).toBeLessThan(PERFORMANCE_TARGETS.RESPONSE_TIME_99TH);
    ...
    test('should handle sustained load over time', async () => {
      ...
      const sustainedSuccessRate = successfulOps.length / sustainedResults.length;
      const avgSustainedResponseTime = successfulOps.reduce((sum, r) => sum + r.responseTime, 0) / successfulOps.length;
      
      // Check for performance degradation over time
      const firstHalf = successfulOps.slice(0, Math.floor(successfulOps.length / 2));
      const secondHalf = successfulOps.slice(Math.floor(successfulOps.length / 2));
      ...
      const performanceDegradation = (secondHalfAvgTime - firstHalfAvgTime) / firstHalfAvgTime;
      ...
      console.log(`  ✅ Sustained success rate: ${(sustainedSuccessRate * 100).toFixed(1)}%`);
  ...
  async function warmUpSystem() {
    ...
    await new Promise(resolve => setTimeout(resolve, LOAD_TEST_CONFIG.GC_WAIT_TIME));
  }
