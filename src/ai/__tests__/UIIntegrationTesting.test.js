...
describe("Phase 6: UI Integration & User Experience Testing", () => {
  ...
  describe("Sparkle Button Integration", () => {
    ...
    test("should handle AI processing workflow smoothly", async () => {
      ...
      const mockOnAutoFill = jest
        .fn()
        .mockImplementation((progressCallback) => {
  ...
  describe("Japanese Localization Testing", () => {
    test("should display all text in Japanese", async () => {
      ...
      const localizationCoverage =
        (foundTexts.length / japaneseTexts.length) * 100;

      expect(localizationCoverage).toBeGreaterThan(
        UX_TARGETS.JAPANESE_LOCALIZATION - 10,
      ); // Allow 10% tolerance
      ...
      console.log(
        `  ✅ Japanese localization coverage: ${localizationCoverage.toFixed(1)}%`,
      );
    });
  ...
  describe("UI Responsiveness Testing", () => {
    test("should maintain UI responsiveness during ML operations", async () => {
      ...
      const avgResponseTime =
        responsivnessMeasurements.reduce((sum, t) => sum + t, 0) /
        responsivnessMeasurements.length;
      ...
      console.log(
        `  ✅ Average UI response time: ${avgResponseTime.toFixed(1)}ms`,
      );
  ...
  describe("Error Handling UI/UX", () => {
    test("should display user-friendly error recovery options", async () => {
      ...
      const errorUIComplete = Object.values(errorElements).every(
        (element) => element,
      );
    ...
    test("should handle network/system failures gracefully in UI", async () => {
      ...
      const mockOnAutoFillNetworkFailure = jest
        .fn()
  ...
  describe("Accessibility Testing", () => {
    test("should meet accessibility standards", async () => {
      ...
      const accessibilityChecks = {
        headerExists: !!screen.getByRole("heading", {
          name: /AI アシスタント/,
        }),
        buttonsHaveLabels: !!screen.getByRole("button", {
          name: /AI 自動入力を実行/,
        }),
      ...
      const accessibilityScore =
        (Object.values(accessibilityChecks).filter((check) => check === true)
          .length /
          Object.keys(accessibilityChecks).length) *
        100;

      expect(accessibilityScore).toBeGreaterThan(
        UX_TARGETS.ACCESSIBILITY_SCORE,
      );
      ...
      console.log(
        `  ✅ Accessibility score: ${accessibilityScore.toFixed(1)}%`,
      );
    });
  });
  ...
  describe("Performance Visual Testing", () => {
    test("should render without visual glitches during state changes", async () => {
      ...
      const avgRenderTime =
        renderingResults.reduce((sum, r) => sum + r.renderTime, 0) /
        renderingResults.length;
