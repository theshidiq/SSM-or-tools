...
describe('Phase 6: Business Rule Compliance Validation', () => {
  ...
  describe('Labor Law Compliance', () => {
    test('should enforce maximum consecutive working days', async () => {
      ...
      const testScenarios = [
        { staffType: '社員', maxDays: 6, description: 'Regular employee consecutive days' },
        { staffType: 'パート', maxDays: 4, description: 'Part-time employee consecutive days' }
      ];
      ...
      for (const scenario of testScenarios) {
        ...
        console.log(`  ✅ ${scenario.description}: ${violations.length} violations`);
      }
    ...
    test('should enforce weekly working hour limits', async () => {
      ...
      testData.staffMembers.forEach(staff => {
        ...
        const weeklyHours = calculateWeeklyHours(staffSchedule, testData.dateRange);
      ...
      console.log(`  ✅ Weekly hour compliance: ${weeklyHoursViolations.length} violations`);
    });
    ...
    test('should enforce mandatory break requirements', async () => {
      ...
      console.log(`  ✅ Break requirement compliance: ${breakViolations.length} violations`);
    });
  });
  ...
  describe('Minimum Staffing Requirements', () => {
    test('should maintain minimum staff per shift type', async () => {
      ...
      console.log(`  ✅ Minimum staffing compliance: ${staffingViolations.length} violations`);
    });
    ...
    test('should ensure skill requirements are met', async () => {
      ...
      reportGenerator.addTestResult('Skill Requirement Compliance', {
        ...
        skillsTracked: Object.values(skillRequirements).flat().filter((v, i, a) => a.indexOf(v) === i).length
      });
      
      console.log(`  ✅ Skill requirement compliance: ${skillViolations.length} violations`);
    });
    ...
    test('should maintain department coverage requirements', async () => {
      ...
      console.log(`  ✅ Department coverage compliance: ${departmentViolations.length} violations`);
    });
  });
  ...
  describe('Workload Balance & Fairness', () => {
    test('should maintain fair workload distribution', async () => {
      ...
      console.log(`  ✅ Workload fairness index: ${(workloadAnalysis.fairnessIndex * 100).toFixed(1)}%`);
      console.log(`  ✅ Coefficient of variation: ${(workloadAnalysis.coefficientOfVariation * 100).toFixed(1)}%`);
    });
    ...
    test('should respect individual staff preferences', async () => {
      ...
      console.log(`  ✅ Preference satisfaction: ${(preferenceCompliance.overallScore * 100).toFixed(1)}%`);
    });
  });
  ...
  describe('Cost Optimization with Compliance', () => {
    test('should optimize costs while maintaining all compliance requirements', async () => {
      ...
      expect(costAnalysis.totalCost).toBeLessThan(costConstraints.maxDailyCost * testData.dateRange.length);
      ...
      console.log(`  ✅ Average daily cost: ¥${costAnalysis.avgDailyCost.toLocaleString()}`);
      console.log(`  ✅ Cost efficiency: ${(costAnalysis.costEfficiencyScore * 100).toFixed(1)}%`);
  ...
  function validateSkillRequirements(schedule, staffMembers, skillRequirements) {
  ...
  function analyzeWorkloadDistribution(schedule, staffMembers, dateRange) {
    const workloads = staffMembers.map(staff => {
      const staffSchedule = schedule[staff.id] || {};
      return Object.values(staffSchedule).filter(shift => shift !== '×').length;
    });
    ...
    const fairnessIndex = (sumTotal * sumTotal) / (workloads.length * sumSquares);
    
    const overworkedStaff = workloads.filter(w => w > avgWorkload * 1.3).length; // 30% over average
  ...
  function createStaffWithPayRates(count) {
    return createTestStaffMembers(count).map(staff => ({
      ...staff,
      hourlyRate: staff.status === '社員' ? 
        1500 + Math.random() * 500 : // ¥1500-2000 for regular
        1000 + Math.random() * 300   // ¥1000-1300 for part-time
    }));
  }
  
  function calculateScheduleCosts(schedule, staffMembers, dateRange, constraints) {
    const shiftHours = { '△': 6, '○': 8, '▽': 6, '×': 0 };
    let totalCost = 0;
