...
describe("Memory Management Testing Suite", () => {
  ...
  beforeAll(async () => {
    ...
    console.log("================================================");
    console.log(
      "Testing tensor cleanup, memory leaks, pooling, and pressure handling",
    );
    console.log("================================================\n");
    ...
    console.log(
      `📊 Initial memory usage: ${(initialMemorySnapshot.heapUsed / 1024 / 1024).toFixed(2)}MB`,
    );
  });
  ...
  describe("TensorFlow Tensor Cleanup Tests", () => {
    test("should track tensor lifecycle correctly", async () => {
      ...
      if (!memoryManager) {
        console.log(
          "ℹ️ Skipping tensor lifecycle test - MemoryManager not available",
        );
        return;
      }
      ...
      console.log(
        `📊 Tensors during operations: ${duringStats.numTensors || 0}`,
      );
      ...
      expect(duringStats.numTensors || 0).toBeGreaterThanOrEqual(
        beforeStats.numTensors || 0,
      );

      if ((duringStats.numTensors || 0) > 0) {
        const cleanupEfficiency =
          ((duringStats.numTensors || 0) - (afterStats.numTensors || 0)) /
          (duringStats.numTensors || 0);
        console.log(
          `🗑️ Tensor cleanup efficiency: ${(cleanupEfficiency * 100).toFixed(1)}%`,
        );
    ...
    test("should dispose of tensors automatically", async () => {
      ...
      console.log(
        `📊 Final memory growth: ${(memoryGrowth / 1024 / 1024).toFixed(2)}MB`,
      );

      expect(result.success).toBe(true);
      expect(memoryGrowth).toBeLessThan(MEMORY_TARGETS.MEMORY_LEAK_THRESHOLD);
      expect(maxMemoryDuringProcessing).toBeLessThan(
        initialMemory + MEMORY_TARGETS.MAX_MEMORY_USAGE,
      );
    });
    ...
    test("should handle tensor disposal errors gracefully", async () => {
      ...
      if (!memoryManager) {
        console.log(
          "ℹ️ Skipping disposal error test - MemoryManager not available",
        );
        return;
      }
  ...
  describe("Memory Leak Detection Tests", () => {
    test("should detect memory leaks in processing cycles", async () => {
      ...
      for (let cycle = 0; cycle < cycleCount; cycle++) {
        ...
        console.log(
          `   📊 Cycle ${cycle + 1} memory growth: ${(cycleGrowth / 1024 / 1024).toFixed(2)}MB`,
        );
      }
      ...
      const averageGrowth =
        memoryGrowths.reduce((a, b) => a + b, 0) / memoryGrowths.length;
      const totalGrowth =
        memoryReadings[memoryReadings.length - 1].endMemory -
        memoryReadings[0].startMemory;

      // Check for consistent memory growth (potential leak)
      const growthTrend = calculateGrowthTrend(memoryGrowths);

      console.log(
        `📈 Average memory growth per cycle: ${(averageGrowth / 1024 / 1024).toFixed(2)}MB`,
      );
      console.log(
        `📊 Total memory growth: ${(totalGrowth / 1024 / 1024).toFixed(2)}MB`,
      );
      console.log(
        `📈 Growth trend: ${growthTrend > 0 ? "increasing" : "stable"}`,
      );

      // Memory leaks should be minimal
      expect(totalGrowth).toBeLessThan(MEMORY_TARGETS.MEMORY_LEAK_THRESHOLD);
      expect(averageGrowth).toBeLessThan(
        MEMORY_TARGETS.MEMORY_LEAK_THRESHOLD / cycleCount,
      );
    });
    ...
    test("should identify memory leak sources", async () => {
      ...
      if (!memoryManager) {
        console.log(
          "ℹ️ Skipping leak source test - MemoryManager not available",
        );
        return;
      }
      ...
      for (const source of potentialLeakSources) {
        ...
        console.log(
          `   📊 ${source.name}: ${sourceSize} items, ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB increase`,
        );
      }

      // Identify significant leak sources
      const significantLeaks = leakSourceResults.filter(
        (r) => r.leakSeverity > 5,
      ); // >5MB

      console.log(
        `🚨 Significant leak sources detected: ${significantLeaks.length}`,
      );
    ...
    test("should monitor memory usage patterns", async () => {
      ...
      const patterns = [
        ...
        { name: "Spike Usage", action: () => createMemorySpike() },
        {
          ...
          action: () => createGradualMemoryIncrease(),
        },
      ...
      for (const pattern of patterns) {
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, monitorDuration / patterns.length),
        );
      }
      ...
      console.log(
        `   Peak usage: ${(analysis.peakUsage / 1024 / 1024).toFixed(2)}MB`,
      );
      console.log(
        `   Average usage: ${(analysis.averageUsage / 1024 / 1024).toFixed(2)}MB`,
      );
  ...
  describe("Tensor Pooling and Reuse Tests", () => {
    test("should implement tensor pooling effectively", async () => {
      ...
      if (!memoryManager) {
        console.log(
          "ℹ️ Skipping tensor pooling test - MemoryManager not available",
        );
        return;
      }
      ...
      for (let i = 0; i < 20; i++) {
        ...
        if (allocation.fromPool) {
          ...
          poolMisses++;
          console.log(
            `   ➕ Pool miss for ${tensorSize.join("x")} tensor (new allocation)`,
          );
        }
    ...
    test("should optimize memory allocation through reuse", async () => {
      ...
      for (const test of allocationTests) {
        ...
        for (let i = 0; i < 50; i++) {
          ...
          const allocation = await simulateTensorAllocation(
            tensorSize,
            test.usePooling,
          );
        ...
        console.log(
          `   ⏱️ ${test.name} time: ${testResult.allocationTime.toFixed(0)}ms`,
        );
        console.log(
          `   📦 ${test.name} memory: ${(testResult.memoryUsed / 1024 / 1024).toFixed(2)}MB`,
        );
      }
      ...
      if (withoutPooling && withPooling) {
        const timeImprovement =
          (withoutPooling.allocationTime - withPooling.allocationTime) /
          withoutPooling.allocationTime;
        const memoryImprovement =
          (withoutPooling.memoryUsed - withPooling.memoryUsed) /
          withoutPooling.memoryUsed;

        console.log(`📈 Pooling improvements:`);
        console.log(
          `   Time improvement: ${(timeImprovement * 100).toFixed(1)}%`,
        );
        console.log(
          `   Memory improvement: ${(memoryImprovement * 100).toFixed(1)}%`,
        );
  ...
  describe("Memory Pressure Detection Tests", () => {
    test("should detect memory pressure conditions", async () => {
      ...
      if (!memoryManager) {
        console.log(
          "ℹ️ Skipping memory pressure test - MemoryManager not available",
        );
        return;
      }
      ...
      memoryManager.onMemoryPressure = (level, stats) => {
        ...
        console.log(
          `   🚨 Memory pressure detected: ${level} (${(stats.currentMemory / 1024 / 1024).toFixed(2)}MB)`,
        );
      };
      ...
      console.log(
        `📊 Memory pressure events detected: ${pressureEvents.length}`,
      );

      // Analyze pressure events
      pressureEvents.forEach((event, index) => {
        console.log(
          `   Event ${index + 1}: ${event.level} pressure at ${(event.stats.currentMemory / 1024 / 1024).toFixed(2)}MB`,
        );
      });
      ...
      if (pressureEvents.length > 0) {
        ...
        expect(pressureEvents[0].stats.currentMemory).toBeGreaterThan(
          MEMORY_TARGETS.MEMORY_PRESSURE_THRESHOLD * 0.8,
        );
      } else {
        console.log(
          "ℹ️ No memory pressure events detected (normal in test environment)",
        );
      }
    });
    ...
    test("should trigger emergency cleanup under memory pressure", async () => {
      ...
      if (!memoryManager) {
        console.log(
          "ℹ️ Skipping emergency cleanup test - MemoryManager not available",
        );
        return;
      }
      ...
      memoryManager.onMemoryCleanup = (cleanupStats) => {
        ...
        console.log(
          `   🗑️ Emergency cleanup #${emergencyCleanups}: ${JSON.stringify(cleanupStats)}`,
        );
      };
      ...
      try {
        await performanceManager.processMLPredictions(
          ...
          (progress) => {
            if (progress.memoryPressure) {
              console.log(
                `   ⚠️ Memory pressure during processing: ${progress.memoryPressure}`,
              );
            }
          },
        );
        ...
        if (emergencyCleanups > 0) {
          const avgEffectiveness =
            cleanupEffectiveness.reduce(
              (sum, stats) => sum + (stats.memoryFreed || 0),
              0,
            ) / cleanupEffectiveness.length;

          console.log(
            `📊 Average cleanup effectiveness: ${(avgEffectiveness / 1024 / 1024).toFixed(2)}MB`,
          );
