...
describe("AIFoundation", () => {
  ...
  describe("Initialization", () => {
    test("should initialize successfully", async () => {
      ...
      jest
        .spyOn(require("../utils/DataExtractor"), "extractAllDataForAI")
        .mockReturnValue({
          success: true,
          data: {
            rawPeriodData: [
  ...
  describe("System Status", () => {
    ...
    test("should return system status when initialized", async () => {
      ...
      expect(
        status.components.constraintManager.totalConstraints,
      ).toBeGreaterThan(0);
    });
  });
  ...
  describe("Error Handling", () => {
    test("should handle method calls before initialization", async () => {
      await expect(aiFoundation.analyzeSchedule(0)).rejects.toThrow(
        "AI Foundation not initialized",
      );

      await expect(
        aiFoundation.validateConstraints({}, [], []),
      ).rejects.toThrow("AI Foundation not initialized");

      await expect(
        aiFoundation.analyzeStaffPreferences("staff_001", {}, []),
      ).rejects.toThrow("AI Foundation not initialized");
    });
    ...
    test("should handle analysis errors gracefully", async () => {
      ...
      await expect(aiFoundation.analyzeSchedule(0)).rejects.toThrow(
        "Data extraction failed: Analysis failed",
      );
    });
  });
});
...
describe("Constraint Engine Integration", () => {
  test("should validate monthly off limits correctly", () => {
    ...
    const monthlyViolations = result.allViolations.filter(
      (v) => v.type === "monthly_off_limit",
    );
  ...
  test("should validate daily limits correctly", () => {
    ...
    const result = validateAllConstraints(scheduleData, staffMembers, [
      new Date("2025-01-21"),
    ]);

    expect(result.valid).toBe(false);

    const dailyViolations = result.allViolations.filter(
      (v) => v.type === "daily_off_limit",
    );
...
describe("Performance Tests", () => {
  ...
  test("should handle large datasets efficiently", async () => {
    ...
    const largeDateRange = Array.from(
      { length: 31 },
