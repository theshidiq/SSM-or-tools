...
import { featureCacheManager, FeatureCacheManager } from '../cache/FeatureCacheManager.js';
    ...
    EnhancedFeatureEngineering: jest.fn().mockImplementation(() => ({
      generateFeatures: jest.fn(() => {
        ...
        return new Promise(resolve => {
          setTimeout(() => {
            resolve(Array(42).fill(0).map(() => Math.random()));
          }, delay);
        });
      })
    }))
...
describe('Phase 2: Feature Cache Performance Validation', () => {
  ...
  describe('Configuration Change Detection and Cache Invalidation', () => {
    test('should invalidate cache when staff configuration changes', () => {
      ...
      const modifiedStaff = [...mockStaff, { id: 'staff-3', name: '新人', status: 'パート' }];
      const invalidated = cacheManager.invalidateOnConfigChange(modifiedStaff, mockSchedule);
    ...
    test('should not invalidate cache when configuration is unchanged', () => {
      ...
      // Same configuration
      const invalidated = cacheManager.invalidateOnConfigChange(mockStaff, mockSchedule);
    ...
    test('should detect schedule data changes', () => {
      ...
      const invalidated = cacheManager.invalidateOnConfigChange(mockStaff, schedule2);
  ...
  describe('Feature Caching Performance', () => {
    test('should achieve <10ms cache hit performance', async () => {
      ...
      const testFeatures = Array(42).fill(0).map(() => Math.random());
    ...
    test('should validate features before caching', () => {
      ...
      // Valid features
      const validFeatures = Array(42).fill(0).map(() => Math.random());
      expect(cacheManager.setFeatures('staff-1', dateKey, validFeatures)).toBe(true);
      
      // Invalid features - not array
      expect(cacheManager.setFeatures('staff-2', dateKey, 'not-array')).toBe(false);
      
      // Invalid features - contains NaN
      const invalidFeatures = [1, 2, NaN, 4, 5];
      expect(cacheManager.setFeatures('staff-3', dateKey, invalidFeatures)).toBe(false);
      
      // Invalid features - contains Infinity
      const infiniteFeatures = [1, 2, Infinity, 4, 5];
      expect(cacheManager.setFeatures('staff-4', dateKey, infiniteFeatures)).toBe(false);
    });
  });
  ...
  describe('Background Precomputation', () => {
    ...
    test('should not start multiple background computations', async () => {
      ...
      cacheManager.startBackgroundPrecomputation(mockStaff, dateRange, mockPeriodData, {});
      expect(cacheManager.isPrecomputing).toBe(true);
  ...
  describe('Phase 2 Performance Goals Validation', () => {
    ...
    test('should maintain performance with multiple staff and dates', async () => {
      ...
      console.log(`Precompute ${mockStaff.length * dateRange.length} entries: ${precomputeTime}ms`);
      ...
      const avgCacheHitTime = cacheHitTime / (mockStaff.length * dateRange.length);
      
      console.log(`${mockStaff.length * dateRange.length} cache hits: ${cacheHitTime}ms (avg: ${avgCacheHitTime.toFixed(1)}ms per hit)`);
