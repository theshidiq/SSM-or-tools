...
export const runBasicDemo = async () => {
  ...
  try {
    ...
    console.log("📊 System Status:", {
      ...
      staffWithPreferences:
        status.components.preferenceManager.staffWithPreferences,
    });

    // Step 3: Analyze current schedule (if data exists)
    if (
      status.lastAnalysisSummary &&
      status.lastAnalysisSummary.totalStaff > 0
    ) {
      ...
      if (analysisResult.success) {
        ...
        if (
          analysisResult.analysis.optimizationAnalysis.opportunities.length > 0
        ) {
          console.log("\n🎯 Optimization Opportunities:");
          analysisResult.analysis.optimizationAnalysis.opportunities.forEach(
            (opp, index) => {
              console.log(`  ${index + 1}. ${opp.description}`);
              console.log(
                `     Impact: Efficiency +${opp.impact.efficiency}%, Fairness +${opp.impact.fairness}%`,
              );
            },
          );
        }
  ...
  try {
    ...
    if (validationResult.totalViolations > 0) {
      ...
      const criticalViolations =
        validationResult.constraintValidation.allViolations.filter(
...
export const runPreferenceAnalysisDemo = async () => {
  ...
  try {
    ...
    if (status.components.preferenceManager.staffWithPreferences === 0) {
      console.log(
        "⚠️ No staff preferences found. This is normal for a fresh installation.",
      );
      console.log(
        "💡 Preferences are automatically created from historical schedule patterns.",
      );
      return;
    }
    ...
    console.log(
      `⭐ Overall Score: ${preferenceResult.overallScore.toFixed(1)}`,
    );
    ...
    if (preferenceResult.recommendations.length > 0) {
      ...
      preferenceResult.recommendations.forEach((rec, index) => {
        console.log(
          `  ${index + 1}. ${rec.suggestion} (Priority: ${rec.priority})`,
        );
      });
    }
...
export const runOptimizationWorkflowDemo = async () => {
  ...
  try {
    ...
    console.log(
      `🚨 Constraint Violations: ${optimizationResult.constraintViolations}`,
    );
    console.log(
      `⭐ Average Preference Score: ${optimizationResult.averagePreferenceScore.toFixed(1)}`,
    );
    console.log(
      `🎯 Optimization Opportunities: ${optimizationResult.optimizationOpportunities.length}`,
    );
    ...
    if (optimizationResult.actionPlan.length > 0) {
      ...
      optimizationResult.actionPlan.forEach((action) => {
        console.log(
          `  Step ${action.step}: ${action.action} (${action.priority} - ${action.estimatedTime})`,
        );
      });
    }
...
export const runDataManagementDemo = async () => {
  ...
  try {
    ...
    console.log(
      `  Staff Groups: ${Object.keys(exportedData.staffGroups.groups || {}).length}`,
    );
    console.log(
      `  Constraints: ${Object.keys(exportedData.constraints.constraints || {}).length}`,
    );
    console.log(
      `  Preferences: ${Object.keys(exportedData.preferences.preferences || {}).length}`,
    );
    ...
    if (importResult.success) {
      ...
      console.log(
        `  Staff Groups: ${newStatus.components.staffGroupManager.totalGroups}`,
      );
      console.log(
        `  Constraints: ${newStatus.components.constraintManager.totalConstraints}`,
      );
    } else {
...
export const runAllDemos = async () => {
  ...
  try {
    ...
    console.log(
      "   You can now integrate these features into your shift scheduling application.",
    );
  } catch (error) {
