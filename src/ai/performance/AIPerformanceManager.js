...
class AIPerformanceManager {
  ...
  applyConfiguration(options) {
    ...
    this.config.maxMemoryMB = Math.max(100, Math.min(1000, this.config.maxMemoryMB));
    this.config.workerTimeout = Math.max(30000, Math.min(600000, this.config.workerTimeout));
  }
  ...
  async initializeComponents(options) {
    ...
    try {
      ...
      if (this.config.enablePerformanceMonitoring) {
        ...
        results.performanceMonitor = await this.components.performanceMonitor.initialize({
      ...
      if (this.config.enableStreaming) {
        ...
        results.streamingManager = await this.components.streamingManager.initialize({
    ...
    try {
      ...
      switch (strategy.type) {
        case 'worker_streaming':
          result = await this.processWithWorkerStreaming(data, enhancedProgressCallback, strategy);
          break;
          
        case 'worker_batch':
          result = await this.processWithWorker(data, enhancedProgressCallback, strategy);
          break;
          
        case 'streaming_fallback':
          result = await this.processWithStreamingFallback(data, enhancedProgressCallback, strategy);
          break;
          
        case 'basic_fallback':
          result = await this.processWithBasicFallback(data, enhancedProgressCallback, strategy);
          break;
      ...
      // Finalize job
      if (performanceJob) {
        this.components.performanceMonitor.endJob(jobId, { success: true, result });
      }
      ...
      // Handle error
      if (performanceJob) {
        this.components.performanceMonitor.endJob(jobId, { success: false, error: error.message });
      }
  ...
  selectProcessingStrategy(data) {
    ...
    if (this.components.workerManager && this.components.streamingManager && 
        dataSize > 1000 && memoryAvailable > 200 * 1024 * 1024) {
      return {
        type: 'worker_streaming',
        reason: 'Large dataset with sufficient memory, using worker + streaming',
        dataSize,
        memoryAvailable
      };
    }
  ...
  async processWithWorkerStreaming(data, progressCallback, strategy) {
    ...
    // Process with worker
    const workerResult = await this.components.workerManager.processMLPredictions(
      data,
      (progress) => {
        progressCallback({
          ...progress,
  ...
  async cancelProcessing() {
    ...
    try {
      ...
      if (this.components.performanceMonitor && this.state.currentJobId) {
        this.components.performanceMonitor.cancelJob(this.state.currentJobId, 'user_cancelled');
      }
    ...
    try {
      ...
      if (this.components.memoryManager) {
        ...
        health.components.memory = {
          status: memoryStats.memoryUtilization > 90 ? 'critical' : 
                 memoryStats.memoryUtilization > 70 ? 'warning' : 'good',
  ...
  generateHealthRecommendations(health) {
    ...
    if (health.components.memory?.utilization > 80) {
      recommendations.push({
        type: 'memory',
        severity: 'warning',
        message: 'メモリ使用量が高くなっています。メモリクリーンアップを実行してください。',
    ...
    if (health.components.performance?.averageProcessingTime > 30000) {
      recommendations.push({
        type: 'performance',
        severity: 'info',
        message: '処理時間が長くなっています。Web Workerの使用を検討してください。',
  ...
  updateSystemMetrics(result, strategy) {
    ...
    if (result.processingTime) {
      const totalTime = this.systemMetrics.averageJobTime * (this.systemMetrics.totalJobs - 1) + result.processingTime;
      this.systemMetrics.averageJobTime = Math.round(totalTime / this.systemMetrics.totalJobs);
    }
  }
