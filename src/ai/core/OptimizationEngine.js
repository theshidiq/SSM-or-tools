...
export class OptimizationEngine {
  ...
  async optimize(scheduleData, staffMembers, dateRange, options = {}) {
    ...
    console.log(
      `🎯 Starting multi-objective optimization with ${goals.length} objectives...`,
    );
    ...
    try {
      ...
      const weights =
        this.weightingSchemes.get(weightingScheme) ||
        this.weightingSchemes.get("balanced");
      ...
      const initialOverallScore = this.calculateWeightedScore(
        initialScores,
        weights,
        goals,
      );
      ...
      console.log(
        `📊 Initial overall score: ${initialOverallScore.toFixed(2)}%`,
      );
      ...
      while (iteration < maxIterations && noImprovementCount < 20) {
        ...
        for (const candidate of candidates) {
          ...
          const candidateOverallScore = this.calculateWeightedScore(
            candidateScores,
            weights,
            goals,
          );
        ...
        if (
          bestCandidate &&
          bestCandidateScore > currentScore + convergenceThreshold
        ) {
          ...
          console.log(
            `📈 Iteration ${iteration}: Score improved by ${improvement.toFixed(2)}% to ${currentScore.toFixed(2)}% using ${bestCandidate.strategy}`,
          );
        } else {
          noImprovementCount++;
        }
        ...
        if (currentScore >= 95) {
          console.log(
            `🎯 Excellent score achieved (${currentScore.toFixed(2)}%), stopping optimization`,
          );
          break;
        }
      }
        ...
        analysis: {
          ...
          averageImprovementPerIteration:
            improvements.length > 0
              ? improvementScore / improvements.length
              : 0,
          objectiveBreakdown: this.analyzeObjectiveContributions(
            finalScores,
            weights,
            goals,
          ),
      ...
      console.log(
        `✅ Optimization completed in ${optimizationTime}ms after ${iteration} iterations`,
      );
      console.log(
        `📊 Score improved from ${initialOverallScore.toFixed(2)}% to ${bestScore.toFixed(2)}% (+${improvementScore.toFixed(2)}%)`,
      );
   ...
   */
  async generateCandidateSolutions(
    currentSchedule,
    staffMembers,
    dateRange,
    options = {},
    ...
    const localImprovement = this.generateLocalImprovement(
      currentSchedule,
      staffMembers,
      dateRange,
    );
    ...
    if (focusObjectives.includes("fairness")) {
      const fairnessAdjustment = await this.generateFairnessAdjustment(
        currentSchedule,
        staffMembers,
        dateRange,
      );
    ...
    if (focusObjectives.includes("preferences")) {
      const preferenceAdjustment = await this.generatePreferenceAdjustment(
        currentSchedule,
        staffMembers,
        dateRange,
      );
    ...
    if (focusObjectives.includes("efficiency")) {
      const efficiencyAdjustment = await this.generateEfficiencyAdjustment(
        currentSchedule,
        staffMembers,
        dateRange,
      );
    ...
    if (focusObjectives.includes("constraint_satisfaction")) {
      const constraintAdjustment = await this.generateConstraintAdjustment(
        currentSchedule,
        staffMembers,
        dateRange,
      );
    ...
    if (iteration % 10 === 0) {
      ...
      const randomPerturbation = this.generateRandomPerturbation(
        currentSchedule,
        staffMembers,
        dateRange,
      );
  ...
  async calculateConstraintScore(schedule, staffMembers, dateRange) {
    try {
      ...
      let totalPenalty = 0;
      totalPenalty +=
        validation.summary.criticalViolations * severityWeights.critical;
      totalPenalty += validation.summary.highViolations * severityWeights.high;
      totalPenalty +=
        validation.summary.mediumViolations * severityWeights.medium;
    ...
    try {
      ...
      const workloads = staffMembers.map((staff) => {
        ...
        return {
          ...
          lateShifts,
          workloadRatio:
            dateRange.length > 0 ? workingDays / dateRange.length : 0,
        };
      });
      ...
      const avgWorkload =
        workloadRatios.reduce((sum, ratio) => sum + ratio, 0) /
        workloadRatios.length;
      const variance =
        workloadRatios.reduce(
          (sum, ratio) => sum + Math.pow(ratio - avgWorkload, 2),
          0,
        ) / workloadRatios.length;
      ...
      const avgOffDays =
        offDayCounts.reduce((sum, count) => sum + count, 0) /
        offDayCounts.length;
      const offDayVariance =
        offDayCounts.reduce(
          (sum, count) => sum + Math.pow(count - avgOffDays, 2),
          0,
        ) / offDayCounts.length;
  ...
  async calculateBusinessPriorityScore(schedule, staffMembers, dateRange) {
    try {
      ...
      const constraintScore = await this.calculateConstraintScore(
        schedule,
        staffMembers,
        dateRange,
      );
  ...
  async calculateAllObjectiveScores(schedule, staffMembers, dateRange, goals) {
    ...
    for (const goal of goals) {
      ...
      if (objective) {
        scores[goal] = await objective.calculate(
          schedule,
          staffMembers,
          dateRange,
        );
  ...
  generateLocalImprovement(schedule, staffMembers, dateRange) {
    ...
    for (let i = 0; i < numChanges; i++) {
      ...
      if (
        improved[randomStaff.id] &&
        improved[randomStaff.id][dateKey] !== undefined
      ) {
  ...
  async generateEfficiencyAdjustment(schedule, staffMembers, dateRange) {
    ...
    dateRange.forEach((date) => {
      ...
      if (workingCount < optimalWorking) {
        ...
        const staffOff = staffMembers.filter(
          (staff) =>
            adjusted[staff.id] &&
            adjusted[staff.id][dateKey] !== undefined &&
      ...
      validation.violations.forEach((violation) => {
        if (violation.type === "staff_group_conflict") {
          ...
          if (
            ...
            details.conflictingMembers.length > 0
          ) {
  ...
  generateRandomPerturbation(schedule, staffMembers, dateRange) {
    ...
    for (let i = 0; i < numChanges; i++) {
      ...
      if (
        perturbed[randomStaff.id] &&
        perturbed[randomStaff.id][dateKey] !== undefined
      ) {
        const possibleShifts = ["", "△", "◇", "×"];
        const newShift =
          possibleShifts[Math.floor(Math.random() * possibleShifts.length)];
    ...
    Object.keys(scores).forEach((objective) => {
      ...
      if (score < 70) {
        recommendations.push({
          ...
          suggestions: this.getObjectiveImprovementSuggestions(
            objective,
            score,
          ),
        });
