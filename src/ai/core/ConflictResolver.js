...
export class ConflictResolver {
  ...
  initializeResolutionStrategies() {
    ...
    this.resolutionStrategies.set(VIOLATION_TYPES.COVERAGE_COMPENSATION_VIOLATION, {
      priority: 900,
   ...
   */
  async resolveAllConflicts(scheduleData, staffMembers, dateRange, options = {}) {
    if (!this.initialized) {
    ...
    try {
      ...
      while (attempts < maxAttempts) {
        ...
        const validation = validateAllConstraints(workingSchedule, staffMembers, dateRange);
        ...
        const prioritizedViolations = this.prioritizeViolations(validation.violations);
          ...
          if (strategy) {
            ...
            if (resolutionResult.success) {
              ...
              resolutionLog.push({
                ...
                description: resolutionResult.description || 'Conflict resolved',
                timestamp: new Date().toISOString()
              });

              console.log(`  ✅ Resolved with ${resolutionResult.changesApplied || 0} changes`);
            } else {
              console.log(`  ❌ Failed to resolve: ${resolutionResult.error || 'Unknown error'}`);
            }
          } else {
        ...
        if (changesInThisAttempt === 0) {
          console.log('⚠️ No changes made in this attempt, stopping resolution');
          break;
        }
      }

      // Final validation
      const finalValidation = validateAllConstraints(workingSchedule, staffMembers, dateRange);
      ...
      const result = {
        ...
        resolutionLog,
        analysis: {
          initialViolations: this.countInitialViolations(scheduleData, staffMembers, dateRange),
          finalViolations: finalValidation.summary?.violationsFound || 0,
          resolutionEfficiency: totalConflictsResolved > 0 ? 
            (totalConflictsResolved / attempts) : 0,
          changeEfficiency: totalChanges > 0 ? 
            (totalConflictsResolved / totalChanges) : 0
        },
      ...
      console.log(`📊 ${totalConflictsResolved} conflicts resolved with ${totalChanges} changes`);
      console.log(`🎯 Final validation: ${finalValidation.valid ? 'VALID' : 'HAS VIOLATIONS'}`);
   ...
   * @returns {Object} Resolution result
   */
  async resolveGroupConflicts(violation, schedule, staffMembers, dateRange, options = {}) {
    ...
    const membersWithStaff = conflictingMembers.map(memberInfo => {
      const memberName = typeof memberInfo === 'string' ? 
        memberInfo.split(' ')[0] : memberInfo;
   ...
   * @returns {Object} Resolution result
   */
  async resolveCoverageCompensation(violation, schedule, staffMembers, dateRange, options = {}) {
   ...
   * @returns {Object} Resolution result
   */
  async resolveInsufficientCoverage(violation, schedule, staffMembers, dateRange, options = {}) {
    const { date, details } = violation;
   ...
   * @returns {Object} Resolution result
   */
  async resolveDailyOffLimit(violation, schedule, staffMembers, dateRange, options = {}) {
    const { date, details } = violation;
    ...
    const staffToChange = staffOff.map(staffName => {
      const staff = staffMembers.find(s => s.name === staffName);
      return staff ? { staff, priority: this.getStaffChangePriority(staff) } : null;
    }).filter(s => s !== null);
   ...
   * @returns {Object} Resolution result
   */
  async resolveDailyEarlyLimit(violation, schedule, staffMembers, dateRange, options = {}) {
    const { date, details } = violation;
   ...
   * @returns {Object} Resolution result
   */
  async resolveMonthlyOffLimit(violation, schedule, staffMembers, dateRange, options = {}) {
   ...
   * @returns {Object} Resolution result
   */
  async resolvePriorityRuleViolation(violation, schedule, staffMembers, dateRange, options = {}) {
   ...
   * @returns {Object} Resolution result
   */
  async resolveProximityPattern(violation, schedule, staffMembers, dateRange, options = {}) {
    const { details } = violation;
    const { triggerStaff, targetStaff, triggerOffDay, proximityRange } = details;
    ...
    for (let offset = -proximityRange; offset <= proximityRange; offset++) {
      ...
      const dateInRange = dateRange.some(d => 
        d.toISOString().split('T')[0] === checkDateKey
      );
    ...
    return { success: false, error: 'Could not find suitable date within proximity range' };
  }
   ...
   * @returns {Object} Resolution result
   */
  async resolveConsecutiveDaysOff(violation, schedule, staffMembers, dateRange, options = {}) {
    const { staffName, details } = violation;
  ...
  async countInitialViolations(scheduleData, staffMembers, dateRange) {
    try {
      const validation = validateAllConstraints(scheduleData, staffMembers, dateRange);
  ...
  getStatus() {
    return {
      ...
      successRate: this.performanceMetrics.totalResolutions > 0 ? 
        (this.performanceMetrics.successfulResolutions / this.performanceMetrics.totalResolutions) * 100 : 0
    };
  }
}