...
export const detectDayOfWeekPreferences = (staffProfile) => {
  ...
  if (dayVariances.length > 0) {
    ...
    preferences.summary.consistencyScore = Math.max(0, 100 - Math.sqrt(variance));
  }
...
export const detectShiftTypePreferences = (staffProfile) => {
  ...
  Object.keys(staffProfile.shiftCounts).forEach(shiftType => {
    ...
    preferences.shiftTypes[shiftType] = {
      ...
      preference: percentage > 40 ? 'preferred' : percentage > 20 ? 'neutral' : 'avoided'
    };
  ...
  const shiftTypeVariances = Object.values(preferences.shiftTypes).map(st => st.percentage);
  if (shiftTypeVariances.length > 0) {
    const mean = shiftTypeVariances.reduce((sum, val) => sum + val, 0) / shiftTypeVariances.length;
    const variance = shiftTypeVariances.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / shiftTypeVariances.length;
    ...
    if (preferences.adaptability.score > 80) {
      ...
      preferences.adaptability.description = 'Works all shift types equally well';
    } else if (preferences.adaptability.score > 60) {
      preferences.adaptability.level = 'adaptable';
      preferences.adaptability.description = 'Works most shift types comfortably';
      ...
      preferences.adaptability.description = 'Strongly prefers specific shift types';
    }
  }
...
export const detectConsecutivePatterns = (staffProfile) => {
  ...
  if (workStreaks.length > 0) {
    patterns.workStreaks.average = workStreaks.reduce((sum, val) => sum + val, 0) / workStreaks.length;
  ...
  if (offStreaks.length > 0) {
    patterns.offStreaks.average = offStreaks.reduce((sum, val) => sum + val, 0) / offStreaks.length;
...
export const detectFrequencyPatterns = (staffProfile) => {
  ...
  const totalPossibleShifts = Object.keys(staffProfile.shiftHistory).length * 31; // Rough estimate
  const totalWorkShifts = staffProfile.totalShifts - (staffProfile.shiftCounts['×'] || 0) - (staffProfile.shiftCounts['off'] || 0);
  
  if (totalPossibleShifts > 0) {
    patterns.workFrequency.percentage = (totalWorkShifts / staffProfile.totalShifts) * 100;
  }
  ...
  Object.keys(staffProfile.shiftHistory).forEach(monthIndex => {
    ...
    const workDays = Object.values(monthSchedule).filter(shift => isWorkingShift(shift)).length;
    ...
    patterns.monthlyTrends[monthIndex] = {
      ...
      workDays,
      workPercentage,
      pattern: workPercentage > 80 ? 'heavy' : workPercentage > 60 ? 'regular' : 'light'
    };
  });

  // Calculate consistency
  if (monthlyWorkPercentages.length > 0) {
    const mean = monthlyWorkPercentages.reduce((sum, val) => sum + val, 0) / monthlyWorkPercentages.length;
    const variance = monthlyWorkPercentages.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / monthlyWorkPercentages.length;
  ...
  if (patterns.workFrequency.consistency < 50) {
    patterns.recommendations.push({
      type: 'consistency',
      message: 'Consider standardizing work frequency for better predictability',
      priority: 'medium'
    });
  }
...
export const detectSeasonalPatterns = (staffProfile) => {
  ...
  Object.keys(quarters).forEach(quarter => {
    if (quarters[quarter].length > 0) {
      ...
      patterns.quarterlyTrends[quarter] = {
        ...
        earlyPercentage: totalShifts > 0 ? (earlyShifts / totalShifts) * 100 : 0,
  ...
  const quarterWorkPercentages = Object.values(patterns.quarterlyTrends).map(q => q.workPercentage);
  if (quarterWorkPercentages.length > 1) {
    const mean = quarterWorkPercentages.reduce((sum, val) => sum + val, 0) / quarterWorkPercentages.length;
    const variance = quarterWorkPercentages.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / quarterWorkPercentages.length;
...
export const recognizeAllPatterns = (staffProfile) => {
  ...
  if (adaptabilityScores.length > 0) {
    const avgAdaptability = adaptabilityScores.reduce((sum, score) => sum + score, 0) / adaptabilityScores.length;
  ...
  const strongPreferences = patterns.patterns.dayOfWeek.strongPreferences.length + 
...
export const recognizePatternsForAllStaff = (staffProfiles) => {
  ...
  Object.values(allPatterns.staffPatterns).forEach(patterns => {
    ...
    if (!allPatterns.aggregateInsights.predictabilityDistribution[predictability]) {
      allPatterns.aggregateInsights.predictabilityDistribution[predictability] = 0;
    }
    allPatterns.aggregateInsights.predictabilityDistribution[predictability]++;

    if (!allPatterns.aggregateInsights.specializationDistribution[specialization]) {
      allPatterns.aggregateInsights.specializationDistribution[specialization] = 0;
    }
  ...
  const rigidStaff = allPatterns.aggregateInsights.adaptabilityDistribution['rigid'] || 0;
  const unpredictableStaff = allPatterns.aggregateInsights.predictabilityDistribution['unpredictable'] || 0;

  if (rigidStaff / totalStaff > 0.3) {
    allPatterns.recommendations.push({
      type: 'team_adaptability',
      message: 'High percentage of staff with rigid patterns. Consider flexibility training.',
      priority: 'medium',
  ...
  if (unpredictableStaff / totalStaff > 0.2) {
    allPatterns.recommendations.push({
      ...
      message: 'Some staff have unpredictable patterns. Consider standardizing schedules.',
      priority: 'high',
      affectedStaff: unpredictableStaff
    });
  }

  console.log(`✅ Pattern recognition completed for ${totalStaff} staff members`);
  return allPatterns;
};