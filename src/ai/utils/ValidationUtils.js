...
export const validateScheduleData = (scheduleData, staffMembers = []) => {
  ...
  const validShiftValues = new Set([
    ...
    '', // Normal shift
    'early', 'late', 'normal', 'off', 'holiday' // Text alternatives
  ]);
      ...
      if (shiftValue !== undefined && shiftValue !== null) {
        if (!validShiftValues.has(shiftValue)) {
          ...
          if (typeof shiftValue === 'string' && shiftValue.length <= 5) {
            warnings.push(`Custom shift value "${shiftValue}" for ${staffId} on ${dateKey}`);
          } else {
            errors.push(`Invalid shift value "${shiftValue}" for ${staffId} on ${dateKey}`);
          }
        }
        totalShifts++;
      }
...
export const validateDateRange = (dateRange) => {
  ...
  if (dates.length > 1) {
    ...
    const expectedDays = Math.floor((lastDate - firstDate) / (24 * 60 * 60 * 1000)) + 1;
...
export const validateConstraint = (constraint) => {
  ...
  const validTypes = ['monthly_limit', 'daily_limit', 'staff_group', 'priority_rule'];
  ...
  switch (constraint.type) {
    ...
    case 'staff_group':
      if (!Array.isArray(constraint.groupMembers) || constraint.groupMembers.length === 0) {
        errors.push('Staff group constraint must have valid groupMembers array');
      }
      break;
...
export const validatePreference = (preference) => {
  ...
  if (preference.confidence && !validConfidenceLevels.includes(preference.confidence)) {
    warnings.push(`Unknown confidence level: ${preference.confidence}`);
  }

  // Validate strength
  if (preference.strength !== undefined) {
    if (typeof preference.strength !== 'number' || 
        preference.strength < 0 || preference.strength > 100) {
  ...
  switch (preference.type) {
    ...
    case 'shift_type':
      ...
      if (preference.shiftType && !validShiftTypes.includes(preference.shiftType)) {
        errors.push(`Invalid shift type: ${preference.shiftType}`);
      }
      break;

    case 'consecutive_pattern':
      if (preference.preferredLength !== undefined && 
          (typeof preference.preferredLength !== 'number' || preference.preferredLength < 1)) {
        errors.push('Consecutive pattern preference must have valid preferredLength');
      }
      break;

    case 'frequency':
      if (preference.targetPercentage !== undefined && 
          (typeof preference.targetPercentage !== 'number' || 
           preference.targetPercentage < 0 || preference.targetPercentage > 100)) {
        errors.push('Frequency preference must have valid targetPercentage (0-100)');
      }
      break;
  }
...
export const validateAnalysisResult = (analysisResult) => {
  ...
  if (analysisResult.success) {
    ...
    if (analysisResult.analysis) {
      const requiredComponents = ['workloadAnalysis', 'dayOfWeekAnalysis', 'optimizationAnalysis'];
  ...
  if (config.aiSettings) {
    ...
    if (aiSettings.confidenceThreshold !== undefined && 
        (typeof aiSettings.confidenceThreshold !== 'number' || 
         aiSettings.confidenceThreshold < 0 || aiSettings.confidenceThreshold > 100)) {
 ...
 * @returns {Object} Validation result
 */
export const validateDataIntegrity = (scheduleData, staffMembers, dateRange) => {
  const errors = [];
  const warnings = [];
  ...
  if (scheduleData && staffMembers && dateRange) {
    ...
    const scheduleStaffIds = Object.keys(scheduleData).filter(id => !id.startsWith('_'));
    const missingStaff = staffIds.filter(id => !scheduleStaffIds.includes(id));
    ...
    const missingDates = dateKeys.filter(dateKey => !allScheduleDates.has(dateKey));
    if (missingDates.length > 0 && missingDates.length < dateKeys.length) {
      warnings.push(`Some dates missing from schedule data: ${missingDates.length} dates`);
    }
  }
...
export const validateStaffMembers = (staffMembers) => {
  ...
  staffMembers.forEach((staff, index) => {
    ...
    if (!staffValidation.isValid) {
      errors.push(`Staff member ${index}: ${staffValidation.errors.join(', ')}`);
    }

    if (staffValidation.warnings.length > 0) {
      warnings.push(`Staff member ${index}: ${staffValidation.warnings.join(', ')}`);
    }
...
export const batchValidate = (items, validateFunction, itemType = 'item') => {
  ...
  if (!Array.isArray(items)) {
    return createValidationResult(false, `${itemType}s must be provided as an array`);
  }
  ...
  items.forEach((item, index) => {
    try {
      ...
      if (validation.warnings.length > 0) {
        warnings.push(`${itemType} ${index}: ${validation.warnings.join(', ')}`);
      }
    } catch (error) {
