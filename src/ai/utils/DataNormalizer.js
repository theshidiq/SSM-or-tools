...
export const normalizeExtractedData = (extractedData) => {
  ...
  try {
    const normalizedData = {
      ...
      data: {
        ...extractedData.data,
        rawPeriodData: extractedData.data.rawPeriodData
          .map((periodData) => normalizePeriodData(periodData))
    ...
    if (normalizedData.data.summary) {
      normalizedData.data.summary.totalPeriods =
        normalizedData.data.rawPeriodData.length;
    }
...
export const normalizeViolation = (violation) => {
  ...
  return {
    ...
    date:
      violation.date && isValidDateKey(violation.date) ? violation.date : null,
    details:
...
export const normalizeSeverity = (severity) => {
  const severityString = String(severity || "")
    .toLowerCase()
    .trim();
