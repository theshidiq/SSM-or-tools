...
export function createTestScheduleData(staffCount = 20, dayCount = 60) {
  ...
  for (let staffIndex = 0; staffIndex < staffCount; staffIndex++) {
    ...
    const maxConsecutive = isRegular ? 6 : 4;
...
export function validateBusinessRules(
  ...
  const workloadVariance =
    ...
    workloads.length;
  results.workloadBalance.score = Math.max(
    0,
    1 - workloadVariance / (avgWorkload * avgWorkload),
  );
  ...
  const totalChecks =
    results.consecutiveDays.total +
    results.minStaffing.total +
    results.skillRequirements.total;
  results.overall =
    totalChecks > 0
      ? Math.max(0, ((totalChecks - totalViolations) / totalChecks) * 100)
      : 100;
...
export function validateTensorFlowMetrics(metrics) {
  ...
  if (metrics.trainingTime > 60000) {
    ...
    validationResults.issues.push(
      "Training time too long: " + metrics.trainingTime + "ms",
    );
  }
...
export function createBenchmarkTestCases() {
  return [
    ...
    },
    {
      ...
      expectedAccuracy: { ml: 0.8, rules: 0.55 },
    },
    {
    ...
    },
    {
      ...
      expectedAccuracy: { ml: 0.9, rules: 0.7 },
    },
    {
      ...
      expectedAccuracy: { ml: 0.88, rules: 0.6 },
    },
  ];
}
...
export function generateUserFeedback(
  ...
  Object.entries(schedule).forEach(([staffId, staffSchedule]) => {
    ...
    Object.entries(staffSchedule).forEach(([dateKey, shiftType]) => {
      if (Math.random() < 0.3) {
        // 30% chance of feedback on any given shift
        ...
        feedback.push({
          ...
          suggestedShift: isPositive
            ? shiftType
            : getAlternativeShift(shiftType),
          reason: isPositive
...
export class TestReportGenerator {
  ...
  generateRecommendations() {
    ...
    const performanceIssues = this.testResults.filter(
      (r) => r.result.executionTime && r.result.executionTime > 5000,
    );
