...
export class MLPipelineDebugger {
  ...
  async debugFullPipeline() {
    ...
    try {
      ...
      const featureResults = await this.debugFeatureEngineering(
        dataResults.data,
      );
      ...
      const trainingResults = await this.debugModelTraining(
        featureResults.trainingData,
      );
  ...
  async debugDataExtraction() {
    try {
      ...
      const requirementsMet = Object.values(minRequirements).every(
        (req) => req,
      );
      ...
      if (!requirementsMet) {
        return {
          ...
          minRequirements,
          details:
            "Need at least 1 period, 2 staff, 50 data points, 10% completeness",
        };
      }
  ...
  async debugFeatureEngineering(data) {
    try {
      ...
      console.log(
        `  📝 Testing with staff: ${firstStaff.name}, date: ${firstDate.toISOString().split("T")[0]}`,
      );
      ...
      console.log(
        `  ✅ Generated ${singleFeatures.length} features (expected: ${MODEL_CONFIG.INPUT_FEATURES.TOTAL})`,
      );
      ...
      const featureAnalysis = {
        ...
        featureCountMatches:
          singleFeatures.length === MODEL_CONFIG.INPUT_FEATURES.TOTAL,
  ...
  async debugModelTraining(trainingData) {
    try {
      ...
      const validLabels = uniqueLabels.every(
        (label) => label >= 0 && label < MODEL_CONFIG.ARCHITECTURE.OUTPUT_SIZE,
      );
      ...
      return {
        ...
        ready:
          trainingAnalysis.minSamplesForTraining &&
          trainingAnalysis.tensorFlowReady,
      };
    } catch (error) {
      return {
        success: false,
   ...
   */
  generateSummary() {
    const { overallStatus, dataExtraction, featureEngineering, modelTraining } =
      this.debugResults;
    ...
    switch (overallStatus) {
      ...
      case "data_extraction_failed":
        const dataIssue =
          dataExtraction.error || "Unknown data extraction error";
        return `Data extraction failed: ${dataIssue}`;

      case "feature_engineering_failed":
        const featureIssue =
          featureEngineering.error || "Unknown feature engineering error";
  ...
  generateRecommendations() {
    ...
    if (overallStatus === "data_extraction_failed") {
      recommendations.push(
        "Ensure historical schedule data exists in localStorage",
      );
      recommendations.push(
        "Verify at least 2 staff members have schedule data across multiple dates",
      );
      recommendations.push(
        "Check that schedule data contains meaningful shift symbols (○, △, ▽, ×)",
      );
    }
    ...
    if (overallStatus === "feature_engineering_failed") {
      if (featureEngineering?.analysis?.featureCountMatches === false) {
        recommendations.push(
          "Fix feature count mismatch in FeatureEngineering.js generateFeatures() method",
        );
        recommendations.push(
          "Ensure all 35 features are properly generated and normalized",
        );
      }

      if (featureEngineering?.analysis?.successRate < 0.8) {
        recommendations.push(
          "Improve feature generation success rate by handling edge cases",
        );
        recommendations.push(
          "Add better data validation in prepareTrainingData()",
        );
      }
    }
    ...
    if (overallStatus === "model_training_failed") {
      recommendations.push(
        "Check TensorFlow.js initialization and WebGL backend availability",
      );
      recommendations.push(
        "Verify training data has sufficient samples (minimum 50-100)",
      );
      recommendations.push(
        "Ensure label values are within expected range [0-4]",
      );
    }
    ...
    if (overallStatus === "all_systems_working") {
      ...
      recommendations.push(
        "Add more sophisticated feature engineering (temporal dependencies, staff interactions)",
      );
      recommendations.push(
