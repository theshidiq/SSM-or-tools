...
export class LogisticRegressionModel {
  ...
  async trainBinaryClassifier(features, binaryLabels, _options) {
    ...
    const accuracy = this.calculateBinaryAccuracy(
      finalPredictions,
      binaryLabels,
    );
  ...
  async predict(features) {
    ...
    const predictions = samples.map((sample) => {
      ...
      for (let cls = 0; cls < this.classWeights.length; cls++) {
        const logit =
          this.dotProduct(sample, this.classWeights[cls]) +
          this.classBiases[cls];
  ...
  async updateFromFeedback(feedbackData) {
    ...
    try {
      ...
      for (let cls = 0; cls < this.classWeights.length; cls++) {
        const binaryLabels = feedbackData.labels.map((label) =>
          label === cls ? 1 : 0,
        );
