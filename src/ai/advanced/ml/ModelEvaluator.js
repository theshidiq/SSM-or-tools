...
export class ModelEvaluator {
  ...
  async evaluateModel(predictions, actualValues, options = {}) {
    ...
    try {
      ...
      console.log(
        `Accuracy: ${(accuracy * 100).toFixed(2)}%, F1: ${(f1Score * 100).toFixed(2)}%`,
      );
  ...
  async evaluateSchedule(schedule, staffMembers, dateRange, constraints = {}) {
    ...
    try {
      ...
      const constraintScore = await this.evaluateConstraintCompliance(
        schedule,
        staffMembers,
        dateRange,
        constraints,
      );
      const fairnessScore = await this.evaluateFairness(
        schedule,
        staffMembers,
        dateRange,
      );
      const satisfactionScore = await this.evaluateStaffSatisfaction(
        schedule,
        staffMembers,
        dateRange,
      );
      const efficiencyScore = await this.evaluateOperationalEfficiency(
        schedule,
        staffMembers,
        dateRange,
      );
      ...
      const analysis = await this.generateScheduleAnalysis(
        schedule,
        staffMembers,
        dateRange,
      );
  ...
  calculateConfidenceMetrics(predictions, actual) {
    ...
    const avgCalibrationError =
      predictions.length > 0 ? calibrationError / predictions.length : 0;
   ...
   */
  async evaluateConstraintCompliance(
    schedule,
    staffMembers,
    dateRange,
    constraints,
    ...
    const complianceRate =
      totalChecks > 0 ? (totalChecks - violations) / totalChecks : 1;
  ...
  async evaluateFairness(schedule, staffMembers, dateRange) {
    ...
    staffMembers.forEach((staff) => {
      const staffSchedule = schedule[staff.id] || {};
      const workDayCount = Object.values(staffSchedule).filter(
        (shift) => shift !== "×",
      ).length;
    ...
    const mean =
      workDays.reduce((sum, days) => sum + days, 0) / workDays.length;
    const variance =
      workDays.reduce((sum, days) => sum + Math.pow(days - mean, 2), 0) /
      workDays.length;
  ...
  async evaluateStaffSatisfaction(schedule, staffMembers, dateRange) {
    ...
    staffMembers.forEach((staff) => {
      ...
      const sundayDaysOff = sundayShifts.filter(
        (shift) => shift === "×",
      ).length;
      if (sundayDaysOff > 0) {
  ...
  async evaluateOperationalEfficiency(schedule, staffMembers, dateRange) {
    ...
    const coverageRate =
      dateRange.length > 0 ? adequatelyCoveredDays / dateRange.length : 1;
  ...
  generateQualityRecommendations(scores) {
    ...
    if (scores.constraints < 80) {
      recommendations.push({
        ...
        priority: "high",
        description:
          "Address constraint violations to improve schedule compliance",
        suggestedActions: [
    ...
    if (scores.satisfaction < 75) {
      recommendations.push({
        ...
        priority: "medium",
        description:
          "Enhance staff satisfaction through better preference matching",
        suggestedActions: [
  ...
  identifyWeaknesses(metrics) {
    ...
    if (Math.abs(metrics.precision - metrics.recall) > 0.2)
      weaknesses.push("Imbalanced precision/recall");
  ...
  generateModelRecommendations(metrics) {
    ...
    if (metrics.precision < 0.8) {
      recommendations.push(
        "Reduce false positives through better classification thresholds",
      );
    }

    if (metrics.recall < 0.8) {
      recommendations.push(
        "Improve feature representation to capture more patterns",
      );
    }
