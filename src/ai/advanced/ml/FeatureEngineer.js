...
export class FeatureEngineer {
  ...
  async extractSampleFeatures(params) {
    ...
    try {
      ...
      if (this.config.patternFeatures) {
        features.push(
          ...this.extractShiftPatternFeatures(staffSchedule, dateKey),
        );
      }
      ...
      if (this.config.workloadFeatures) {
        features.push(
          ...this.extractWorkloadFeatures(staffSchedule, monthData, staffId),
        );
      }
      ...
      if (this.config.constraintFeatures) {
        features.push(
          ...this.extractConstraintFeatures(
            staffId,
            dateKey,
            monthData,
            historicalData,
      ...
      if (this.config.preferenceFeatures) {
        features.push(
          ...this.extractPreferenceFeatures(staffId, dateKey, historicalData),
        );
      }
    } catch (error) {
  ...
  async extractPredictionSampleFeatures(params) {
    ...
    try {
      ...
      if (this.config.workloadFeatures) {
        features.push(
          ...this.extractCurrentWorkloadFeatures(
            staffId,
            existingSchedule,
            dateRange,
          ),
      ...
      if (this.config.constraintFeatures) {
        features.push(
          ...this.extractCurrentConstraintFeatures(
            staffId,
            dateKey,
            existingSchedule,
            staffMembers,
      ...
      if (this.config.seasonalFeatures) {
        features.push(
          ...this.extractCurrentSeasonalFeatures(date, seasonalContext),
        );
      }
  ...
  extractStaffFeatures(staffId, historicalData) {
    ...
    if (staffInfo) {
      ...
      // Department
      const departmentEncoding = this.encodeDepartment(
  ...
  extractShiftPatternFeatures(staffSchedule, currentDateKey) {
    ...
    const consecutiveWorkDays = this.getConsecutiveWorkDays(
      staffSchedule,
      currentDateKey,
    );
    ...
    const daysSinceLastOff = this.getDaysSinceLastDayOff(
      staffSchedule,
      currentDateKey,
    );
  ...
  extractWorkloadFeatures(staffSchedule, monthData, staffId) {
    ...
    // Monthly workload
    const totalWorkDays = Object.values(staffSchedule).filter(
      (shift) => shift !== "×",
    ).length;
    ...
    const relativeWorkload =
      averageWorkDays > 0 ? totalWorkDays / averageWorkDays : 1;
  ...
  extractConstraintFeatures(staffId, dateKey, monthData, historicalData) {
    ...
    const groupMemberships = this.getStaffGroupMemberships(
      staffId,
      historicalData,
    );
  ...
  extractPreferenceFeatures(staffId, dateKey, historicalData) {
    ...
    const dayOffPreference = this.getDayOffPreference(
      staffId,
      dayOfWeek,
      historicalData,
    );
    ...
    const shiftPreference = this.getShiftTypePreference(
      staffId,
      dayOfWeek,
      historicalData,
    );
   ...
   */
  extractCurrentConstraintFeatures(
    staffId,
    dateKey,
    existingSchedule,
    staffMembers,
  ...
  async processFeatures(features, useExistingParams = false) {
    ...
    const scaledFeatures = await this.applyScaling(
      paddedFeatures,
      useExistingParams,
    );

    // Apply categorical encoding
    const encodedFeatures = await this.applyCategoricalEncoding(
      scaledFeatures,
      useExistingParams,
    );
  ...
  async applyScaling(features, useExistingParams) {
    ...
    for (let featureIdx = 0; featureIdx < features[0].length; featureIdx++) {
      const featureName =
        this.featureNames[featureIdx] || `feature_${featureIdx}`;
      ...
      if (featureType === "numerical") {
        ...
        if (useExistingParams && this.scalingParameters.has(featureName)) {
          ...
          for (
            let sampleIdx = 0;
            sampleIdx < scaledFeatures.length;
            sampleIdx++
          ) {
          ...
          const params = this.calculateScalingParameters(
            featureValues,
            this.config.numericalScaling,
          );
  ...
  calculateAverageWorkDays(monthData) {
    ...
    return workDayCounts.length > 0
      ? workDayCounts.reduce((sum, count) => sum + count, 0) /
