...
export class MLIntegrationTest {
    ...
    const tests = [
      {
        ...
        test: this.testDataExtraction.bind(this),
      },
      {
        name: "TensorFlow Initialization Test",
        test: this.testTensorFlowInit.bind(this),
      },
      { name: "Model Training Test", test: this.testModelTraining.bind(this) },
      {
        name: "Prediction Generation Test",
        test: this.testPredictionGeneration.bind(this),
      },
      {
        name: "Hybrid Predictor Test",
        test: this.testHybridPredictor.bind(this),
      },
  ...
  async testDataExtraction() {
    ...
    console.log(
      `✅ Found ${periodsFound} periods, ${staffCount} staff profiles`,
    );
  ...
  async testTensorFlowInit() {
    ...
    console.log(
      `✅ TensorFlow initialized, model params: ${modelInfo.totalParams}`,
    );
  ...
  async testModelTraining() {
    ...
    if (!trainingResult || !trainingResult.success) {
      throw new Error(
        `Training failed: ${trainingResult?.error || "Unknown error"}`,
      );
    }
  ...
  async testPredictionGeneration() {
    ...
    dateRange.slice(0, 5).forEach((date) => {
    ...
    const predictionCount = Object.keys(predictions.predictions).reduce(
      (count, staffId) => {
  ...
  async testHybridPredictor() {
    ...
    console.log(
      `✅ Hybrid prediction completed using ${result.metadata.method}`,
    );
  ...
  async testErrorHandling() {
    ...
    const errorResult = await aiErrorHandler.handleError(
      ...
      "test_context",
      {
        ...
        staffMembers: [
          { id: "staff1", name: "テストスタッフ", status: "社員" },
        ],
