...
class ConfigurationCacheManager {
  ...
  async _doInitialize() {
    ...
    try {
      ...
      console.log(`✅ Configuration Cache Manager initialized in ${initTime}ms`);
      
    } catch (error) {
      console.error('❌ Failed to initialize Configuration Cache Manager:', error);
      throw error;
    }
  }
  ...
  async preloadAllConfigurations() {
    ...
    try {
      // Load constraint engine configurations
      const { getAllConfigurations } = await import('../constraints/ConstraintEngine');
      const configurations = await getAllConfigurations();
      
      // Cache individual configuration types
      this.cache.set(this.CACHE_KEYS.STAFF_GROUPS, configurations.staffGroups || {});
      this.cache.set(this.CACHE_KEYS.DAILY_LIMITS, configurations.dailyLimits || {});
      this.cache.set(this.CACHE_KEYS.PRIORITY_RULES, configurations.priorityRules || []);
      this.cache.set(this.CACHE_KEYS.BUSINESS_RULES, configurations.businessRules || {});
      this.cache.set(this.CACHE_KEYS.CONSTRAINT_CONFIG, configurations.constraintConfig || {});
      this.cache.set(this.CACHE_KEYS.SYSTEM_SETTINGS, configurations.systemSettings || {});
      ...
      console.error('❌ Failed to pre-load configurations:', error);
      // Set empty fallback configurations
  ...
  getConfiguration(type = 'full') {
    const cacheKey = type === 'full' ? this.CACHE_KEYS.FULL_CONFIG : 
    ...
    if (this.cache.has(cacheKey)) {
      ...
      console.log(`⚡ Cache HIT for ${type} (${this.metrics.cacheHits} hits total)`);
      return config;
    }

    this.metrics.cacheMisses++;
    console.warn(`💥 Cache MISS for ${type} (${this.metrics.cacheMisses} misses total)`);
    return null;
  }
  ...
  async refreshCache(changedType = null) {
    console.log(`🔄 Refreshing configuration cache${changedType ? ` for ${changedType}` : ''}...`);
  ...
  async refreshSpecificConfiguration(type) {
    const cacheKey = this.CACHE_KEYS[type.toUpperCase().replace(/[^A-Z_]/g, '_')];
    if (!cacheKey) return;

    try {
      const { getSpecificConfiguration } = await import('../constraints/ConstraintEngine');
  ...
  async validateCacheIntegrity() {
    ...
    try {
      ...
      const { getConfigurationChecksum } = await import('../constraints/ConstraintEngine');
      const currentChecksum = await getConfigurationChecksum();
      const cachedConfig = this.cache.get(this.CACHE_KEYS.FULL_CONFIG);
      
      if (!cachedConfig || !cachedConfig._checksum || cachedConfig._checksum !== currentChecksum) {
  ...
  updateAverageFetchTime(newTime) {
    if (this.metrics.averageFetchTime === 0) {
      ...
      this.metrics.averageFetchTime = (this.metrics.averageFetchTime + newTime) / 2;
    }
  }
