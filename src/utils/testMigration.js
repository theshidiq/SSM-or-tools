...
import { performStaffMigration, hasLocalStorageStaffData } from './staffMigrationUtils';
...
export const createSampleLocalStorageData = () => {
  ...
  try {
    ...
    for (let period = 0; period < 3; period++) {
      ...
      if (period < 2) {
        localStorage.setItem(staffKey, JSON.stringify(sampleStaffData.periodBased));
      }
      
      // Add optimized data for period 0 (should be preferred)
      if (period === 0) {
        localStorage.setItem(optimizedStaffKey, JSON.stringify(sampleStaffData.periodBased));
      }
    }
    
    // Legacy data
    localStorage.setItem('staffMembers', JSON.stringify(sampleStaffData.legacy));
    
    // Injected data
    for (let period = 0; period < 2; period++) {
      localStorage.setItem(`staff_members_${period}`, JSON.stringify(sampleStaffData.injected));
    }
    
    // Additional test data with different formats
    localStorage.setItem('staff-3', JSON.stringify([
...
export const runMigrationTest = async (options = {}) => {
  ...
  console.log(`Settings: createSampleData=${createSampleData}, dryRun=${dryRun}, cleanupAfter=${cleanupAfter}`);
  ...
  try {
    ...
    console.log(`📋 Pre-migration check: Found ${dataCheck.count} data sources`);
...
if (typeof window !== 'undefined') {
  ...
  console.log('Available methods: dryRun(), migrate(), createData(), cleanup(), check()');
}