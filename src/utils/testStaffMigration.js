...
export const testUUIDGeneration = () => {
  ...
  transformedData.forEach((staff, index) => {
    ...
    if (uuidRegex.test(staff.id)) {
      ...
      results.errors.push(
        `Staff ${index} (${staff.name}): Invalid UUID format: ${staff.id}`,
      );
    }
...
export const testMigrationWorkflow = async () => {
  ...
  try {
    ...
    const dryRunResult = await migrateStaffToDatabase(transformedData, {
      dryRun: true,
    });
...
export const runAllMigrationTests = async () => {
  ...
  try {
    ...
    console.log("=".repeat(50));
    console.log(
      `UUID Generation: ${results.uuid.errors.length === 0 ? "✅ PASS" : "❌ FAIL"}`,
    );
    console.log(
      `Database Operations: ${results.database.success ? "✅ PASS" : "❌ FAIL"}`,
    );
    console.log(
      `Migration Workflow: ${results.workflow.success ? "✅ PASS" : "❌ FAIL"}`,
    );
    console.log(
    ...
    if (!results.overall) {
      ...
      if (!results.workflow.success) {
        console.log(
          "Workflow Error:",
          results.workflow.error || results.workflow.errors,
        );
      }
    }
  } catch (error) {
...
if (typeof window !== "undefined") {
  ...
  console.log(
    "🧪 Staff Migration Tests loaded! Use window.testStaffMigration.runAllTests() to run all tests.",
  );
}
