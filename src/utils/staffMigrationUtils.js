  ...
  try {
    ...
    for (let period = 0; period < 6; period++) {
      ...
      if (
        ...
        localStorage.getItem(optimizedStaffKey)
      ) {
    ...
    if (
      localStorage.getItem(STAFF_STORAGE_KEYS.LEGACY_STAFF) ||
    ...
    for (let period = 0; period < 6; period++) {
      ...
      if (staffData || optimizedStaffData) {
        ...
        try {
          ...
          console.warn(
            `Failed to parse staff data for period ${period}:`,
            parseError,
          );
        }
      }
      ...
      if (injectedData) {
        try {
          ...
          console.warn(
            `Failed to parse injected staff data for period ${period}:`,
            parseError,
          );
        }
      }
...
export const transformStaffDataForDatabase = (localStaffData) => {
  if (
    !localStaffData ||
    (!Array.isArray(localStaffData) && typeof localStaffData !== "object")
  ) {
    return [];
  }
  ...
  if (Array.isArray(localStaffData)) {
    return localStaffData.map((staff, index) =>
      transformStaffMember(staff, index),
    );
  }

  // Handle object format (some legacy cases)
  if (typeof localStaffData === "object") {
    return Object.values(localStaffData).map((staff, index) =>
      transformStaffMember(staff, index),
    );
  }
...
const transformStaffMember = (staff, index) => {
  ...
  return {
    ...
    staff_order:
      typeof staff.staff_order !== "undefined"
        ? staff.staff_order
        : typeof staff.order !== "undefined"
          ? staff.order
          : index,
  ...
  try {
    ...
    staffToMigrate.forEach((staff) => {
      const isDuplicate = existingStaff.some(
        (existing) =>
          existing.name === staff.name && existing.position === staff.position,
      );
...
export const backupLocalStorageStaffData = () => {
  try {
    ...
    const hasActualData =
      Object.keys(backupData.periodBased).length > 0 ||
...
export const performStaffMigration = async (options = {}) => {
  ...
  try {
    ...
    console.log(
      `📊 Found staff data in localStorage: ${localStorageCheck.foundKeys.join(", ")}`,
    );
    ...
    console.log(
      `📝 Prepared ${uniqueStaff.length} unique staff members for migration`,
    );
  ...
  try {
    ...
    Object.entries(backup.periodBased || {}).forEach(([period, periodData]) => {
      localStorage.setItem(
        periodData.storageKey,
        JSON.stringify(periodData.data),
      );
      restoredCount++;
    });
    ...
    if (backup.legacy) {
      localStorage.setItem(
        backup.legacy.storageKey,
        JSON.stringify(backup.legacy.data),
      );
      restoredCount++;
    }

    console.log(
      `🔄 Restored ${restoredCount} staff data entries from backup: ${backupKey}`,
    );
    return {
      success: true,
      restoredCount,
