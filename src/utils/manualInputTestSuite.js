...
export const manualInputTestSuite = {
  ...
  async testBasicManualInput(updateShift, periodIndex = 0) {
    ...
    try {
      ...
      results.persistenceCheck =
        await dataIntegrityMonitor.checkManualInputPersistence(
          periodIndex,
          testStaffId,
          testDateKey,
          testValue,
      ...
      results.dataIntegrityCheck =
        dataValidation.validateScheduleStructure(scheduleData);
    ...
    try {
      ...
      for (const entry of testEntries) {
        try {
          ...
          results.issues.push(
            `Failed to input ${entry.staffId}/${entry.dateKey}: ${error.message}`,
          );
        }
      }
  ...
  async testRefreshPersistence(periodIndex = 0) {
    ...
    try {
      ...
      console.log(
        "📝 Pre-refresh data captured. Manual refresh required to complete test.",
      );
      console.log(
        "💡 To complete: Refresh page and run `manualInputTestSuite.verifyPostRefreshData(${periodIndex})`",
      );
    } catch (error) {
    ...
    try {
      ...
      if (preRefreshData) {
        ...
        const scheduleMatches =
          JSON.stringify(results.postRefreshData.schedule) ===
          JSON.stringify(preRefreshData.schedule);
        const staffMatches =
          JSON.stringify(results.postRefreshData.staff) ===
    ...
    try {
      ...
      if (window.exportToCSV) {
        try {
          ...
          results.csvExport = {
            success: true,
            dataLength: csvData ? csvData.length : 0,
          };
        } catch (error) {
  ...
  testStatisticsIntegration(schedule, staffMembers, dateRange) {
    ...
    try {
      if (window.generateStatistics) {
        const statistics = window.generateStatistics(
          schedule,
          staffMembers,
          dateRange,
        );
  ...
  async runCompleteTestSuite(
    ...
    results.tests.basicInput = await this.testBasicManualInput(
      updateShift,
      periodIndex,
    );
    results.summary.totalTests++;
    if (results.tests.basicInput.issues.length === 0)
      results.summary.passedTests++;
    else results.summary.failedTests++;

    // Test 2: Bulk manual input
    results.tests.bulkInput = await this.testBulkManualInput(
      updateShift,
      periodIndex,
    );
    results.summary.totalTests++;
    if (results.tests.bulkInput.issues.length === 0)
      results.summary.passedTests++;
    else results.summary.failedTests++;

    // Test 3: Export integration
    results.tests.exportIntegration = await this.testExportIntegration(
      schedule,
      staffMembers,
      dateRange,
    );
    results.summary.totalTests++;
    if (results.tests.exportIntegration.issues.length === 0)
      results.summary.passedTests++;
    else results.summary.failedTests++;

    // Test 4: Statistics integration
    results.tests.statisticsIntegration = this.testStatisticsIntegration(
      schedule,
      staffMembers,
      dateRange,
    );
    results.summary.totalTests++;
    if (results.tests.statisticsIntegration.issues.length === 0)
      results.summary.passedTests++;
    else results.summary.failedTests++;
  ...
  generateTestReport(results) {
    ...
    const passRate = ((summary.passedTests / summary.totalTests) * 100).toFixed(
      1,
    );
