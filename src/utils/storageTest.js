...
export const runStorageTests = () => {
  ...
  try {
    ...
    const testStaffData = [{ id: 'staff-1', name: 'Test Staff', position: 'Server' }];
    ...
    console.log('✅ Schedule storage:', retrievedSchedule && Object.keys(retrievedSchedule).length > 0 ? 'PASS' : 'FAIL');
    console.log('✅ Staff storage:', retrievedStaff && retrievedStaff.length > 0 ? 'PASS' : 'FAIL');
    ...
    console.log('✅ Batch queueing:', afterQueue > initialPending ? 'PASS' : 'FAIL');
    ...
    console.log('✅ Legacy detection:', typeof hasLegacy === 'boolean' ? 'PASS' : 'FAIL');
...
export const benchmarkPerformance = (iterations = 100) => {
  ...
  try {
    ...
    for (let i = 0; i < iterations; i++) {
      localStorage.setItem(`direct-test-${i}`, JSON.stringify({ iteration: i, data: 'test' }));
    }
    ...
    console.table({
      ...
      'Optimized Storage': {
        ...
        'Total (ms)': (results.optimized.write + results.optimized.read).toFixed(2)
      },
      'Improvement': {
        'Write (%)': (((results.direct.write - results.optimized.write) / results.direct.write) * 100).toFixed(1),
        'Read (%)': (((results.direct.read - results.optimized.read) / results.direct.read) * 100).toFixed(1),
        'Total (%)': ((((results.direct.write + results.direct.read) - (results.optimized.write + results.optimized.read)) / (results.direct.write + results.direct.read)) * 100).toFixed(1)
      }
    });
