...
describe("Backup Staff Database Integration", () => {
  ...
  describe("BackupStaffService Integration Methods", () => {
    test("loadBackupAssignments loads from ConfigurationService", async () => {
      ...
      const loadedAssignments =
        await backupStaffService.loadBackupAssignments();
    ...
    test("saveBackupAssignments saves via ConfigurationService", async () => {
      ...
      const success =
        await backupStaffService.saveBackupAssignments(testAssignments);
      expect(success).toBe(true);
    ...
    test("addBackupAssignment creates new assignment", async () => {
      const success = await backupStaffService.addBackupAssignment(
        "staff-1",
        "group-1",
        {
          assignmentType: "regular",
    ...
    test("removeBackupAssignment removes assignment by staffId and groupId", async () => {
      ...
      // Then remove it
      const success = await backupStaffService.removeBackupAssignment(
        "staff-1",
        "group-1",
      );
      expect(success).toBe(true);
  ...
  describe("localStorage Migration", () => {
    test("migrateFromLocalStorage migrates legacy data", async () => {
      ...
      const migratedAssignments =
        await backupStaffService.migrateFromLocalStorage();
  ...
  describe("Error Handling", () => {
    ...
    test("handles duplicate assignment prevention", async () => {
      ...
      const success1 = await backupStaffService.addBackupAssignment(
        "staff-1",
        "group-1",
      );
      ...
      const success2 = await backupStaffService.addBackupAssignment(
        "staff-1",
        "group-1",
      );
  ...
  describe("Data Consistency", () => {
    test("internal maps stay in sync with saved data", async () => {
      ...
      // Check internal maps
      expect(
        backupStaffService.getStaffBackupAssignments("staff-1"),
      ).toHaveLength(2);
